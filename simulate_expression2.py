...
class main:
    ...
    def start(self):
        ...
        eqtl_df = pd.DataFrame(
            {"SNPName": variant_labels, "ProbeName": probe_labels, "FDR": 0}
        )
        print(eqtl_df)
        ...
        for i in range(2):
            geno_m += (
                np.random.uniform(0, 1, size=(n_probes, n_samples)) < maf_m
            ).astype(np.uint8)
        ...
        if self.low_memory:
            ...
            for probe_id in range(n_probes):
                for sample_id in range(n_samples):
                    expr = (
                        random_m[probe_id, sample_id, 0] * std_m[probe_id, 0]
                        + beta_m[probe_id, 0]
                    )
                    expr += (
                        random_m[probe_id, sample_id, 1] * std_m[probe_id, 1]
                        + beta_m[probe_id, 1]
                    ) * geno_m[probe_id, sample_id]
                    for covariate_id in range(n_covariates):
                        expr += (
                            random_m[probe_id, sample_id, 2 + covariate_id]
                            * std_m[probe_id, 2 + covariate_id]
                            + beta_m[probe_id, 2 + covariate_id]
                        ) * cov_m[sample_id, covariate_id]
                        expr += (
                            (
                                random_m[
                                    probe_id, sample_id, 2 + n_covariates + covariate_id
                                ]
                                * std_m[probe_id, 2 + n_covariates + covariate_id]
                                + beta_m[probe_id, 2 + n_covariates + covariate_id]
                            )
                            * geno_m[probe_id, sample_id]
                            * cov_m[sample_id, covariate_id]
                        )
                    expr += (
                        random_m[probe_id, sample_id, 2 + (n_covariates * 2)]
                        * std_m[probe_id, 2 + (n_covariates * 2)]
                        + beta_m[probe_id, 2 + (n_covariates * 2)]
                    )
            ...
            beta_m = np.repeat(
                stats_df.loc[:, beta_columns].to_numpy()[:, np.newaxis, :],
                n_samples,
                axis=1,
            )
            std_m = np.repeat(
                stats_df.loc[:, std_columns].to_numpy()[:, np.newaxis, :],
                n_samples,
                axis=1,
            )
            ...
            model_m = np.ones(
                (n_probes, n_samples, n_terms), dtype=np.float64
            )  # initialise with 1's for the intercept and noise
            model_m[:, :, 1] = geno_m  # genotype term
            model_m[:, :, 2 : (2 + n_covariates)] = np.repeat(
                cov_m[np.newaxis, :, :], n_probes, axis=0
            )  # simulated covariates
            model_m[:, :, (2 + n_covariates) : (2 + (n_covariates * 2))] = (
                model_m[:, :, [1]] * model_m[:, :, 2 : (2 + n_covariates)]
            )  # interactions
        ...
        print("Save data")
        self.save_file(
            df=eqtl_df, outpath=os.path.join(self.outdir, "eQTLProbesProbeLevel.txt.gz")
        )
        self.save_file(
            df=pd.DataFrame(cov_m, index=sample_labels, columns=covariate_labels).T,
            outpath=os.path.join(self.outdir, "simulated_covariates.txt.gz"),
        )
        self.save_file(
            df=pd.DataFrame(geno_m, index=variant_labels, columns=sample_labels),
            outpath=os.path.join(self.outdir, "genotype_table.txt.gz"),
        )
        self.save_file(
            df=pd.DataFrame(expr_m, index=probe_labels, columns=sample_labels),
