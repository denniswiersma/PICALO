...
class main:
    def __init__(self):
        ...
        self.data = Data(
            eqtl_path=getattr(arguments, "eqtl"),
            ...
            tech_covariate_with_inter_path=getattr(
                arguments, "tech_covariate_with_inter"
            ),
    ...
    def start(self):
        ...
        tcov_inter_m, tcov_inter_labels = self.load_tech_cov(
            df=get_tcov_inter_df, name="tech. cov. with interaction", std_df=std_df
        )
        ...
        if self.conditional:
            ...
            for cov_index, covariate in enumerate(covariates):
                ...
                cov_ieqtl_results = self.map_interactions(
                    expr_m=expr_m,
                    corr_m=corr_m,
                ...
                self.save_results(
                    data_m=cov_ieqtl_results[covariate],
                    covariate=covariate,
                    eqtl_m=eqtl_m,
                    prefix="\t  ",
                )
            ...
            ieqtl_results = self.map_interactions(
                expr_m=expr_m,
                corr_m=corr_m,
            ...
            for covariate in covariates:
                self.save_results(
                    data_m=ieqtl_results[covariate],
                    covariate=covariate,
                    eqtl_m=eqtl_m,
                    prefix="  ",
                )
    ...
    def map_interactions(
        ...
        corrected_expr_m = remove_covariates(
            y_m=expr_m, X_m=corr_m, X_inter_m=corr_inter_m, inter_m=geno_m, log=self.log
        )

        self.log.info(
            "{}Force normalise the expression matrix and covariates".format(prefix)
        )
        ...
        ieqtl_results = {
            cov: np.empty((n_eqtls, 10), dtype=np.float64) for cov in covariates
        }
        ...
        for eqtl_index in range(n_eqtls):
            ...
            for cov_index, cov in enumerate(covariates):
                ...
                ieqtl_p_value = calc_p_value(
                    rss1=rss_null, rss2=rss_alt, df1=3, df2=4, n=n
                )

                # Save results.
                ieqtl_results[cov][eqtl_index, :] = np.hstack(
                    (np.array([n]), ieqtl_betas, ieqtl_std, np.array([ieqtl_p_value]))
                )
    ...
    def save_results(self, data_m, covariate, eqtl_m, prefix=""):
        ...
        self.log.info(
            "{}{} has {:,} significant ieQTLs (FDR <{})".format(
                prefix, covariate, n_hits, self.ieqtl_alpha
            )
        )
        ...
        save_dataframe(
            ...
            outpath=os.path.join(
                self.outdir, "{}{}.txt.gz".format(covariate, appendix)
            ),
            header=True,
            index=False,
            log=self.log,
