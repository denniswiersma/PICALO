...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            outdir, "run_PICALO_with_multiple_start_pos", self.outfolder
        )
        self.covariates_outdir = os.path.join(self.outdir, "covariates")
        self.tech_covariate_with_inter_outdir = os.path.join(
            self.outdir, "tech_covariate_with_inter"
        )
        for dir in [
            self.outdir,
            self.covariates_outdir,
            self.tech_covariate_with_inter_outdir,
        ]:
        ...
        self.max_end_time = (
            int(time.time()) + ((getattr(arguments, "max_runtime") * 60) - 5) * 60
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="The path to the technical covariate matrix. " "Default: None.",
        )
        parser.add_argument(
            "-tci",
        ...
        parser.add_argument(
            ...
            type=int,
            default=60,
            help="The print interval time in seconds. " "Default: 60.",
        )
        parser.add_argument(
            "-mr",
            "--max_runtime",
    ...
    def start(self):
        ...
        for covariate in covariates:
            outpath = os.path.join(
                self.covariates_outdir, "{}.txt.gz".format(covariate)
            )
        ...
        print(
            "Running PICALO with {} covariates as start positions".format(n_covariates)
        )
        ...
        for i in range(self.n_components):
            ...
            for covariate, covariate_path in covariates.items():
                jobfile_path, job_name = self.create_job_file(
                    name=name,
            ...
            for job_name, jobfile_path in zip(job_names, jobfile_paths):
                if not os.path.exists(
                    os.path.join(
                        self.picalo_path, "output", job_name, "SummaryStats.txt.gz"
                    )
                ):
            ...
            while True:
                ...
                if (
                    ...
                    or (now_time - last_print_time) >= self.print_interval
                    or n_completed == n_covariates
                ):
                    print(
                        "\t{:,}/{:,} jobs finished [{:.2f}%]".format(
                            n_completed,
                            n_covariates,
                ...
                if n_completed == n_covariates:
                    ...
                    print(
                        "\t\tAll jobs are finished in {} minute(s) and "
            ...
            info_df_m, summary_stats_df = self.combine_picalo_info_files(
                job_names=job_names
            )
            print("Summary stats:")
            ...
            self.save_file(
                df=summary_stats_df,
                outpath=os.path.join(pic_output_dir, "SummaryStats.txt.gz"),
            )
            self.save_file(
                df=pics_df, outpath=os.path.join(pic_output_dir, "PICBasedOnPCX.txt.gz")
            )
            print("")

            print("  Selecting top PIC")
            top_covariate, _, _ = summary_stats_df.loc[
                summary_stats_df["start"].idxmax(), :
            ]
            ...
            for group_index, group_indices in pic_groups.items():
                ...
                if len(group_indices) > 1:
                    ...
                    overlapping_ieqtls = list(
                        group_interaction_df.loc[
                            group_interaction_df.max(axis=1) <= 0.05, :
                        ].index
                    )
                    print(
                        "\t\tOverlapping ieQTLs: {} [N={:,}]".format(
                            ", ".join(overlapping_ieqtls), len(overlapping_ieqtls)
                        )
                    )
            print("")
            ...
            for group_index, group_indices in pic_groups.items():
                group_summary_stats_df = summary_stats_df.loc[
                    summary_stats_df["covariate"].isin(group_indices), :
                ]
                print(
                    "\tGroup{}:\tavg. start: {:.2f}\tavg. end: {:.2f}".format(
                        group_index,
                        group_summary_stats_df["start"].mean(),
                        group_summary_stats_df["end"].mean(),
                    )
                )
                group_top_covariate_index = group_summary_stats_df["start"].idxmax()
                group_top_covariate, _, _ = group_summary_stats_df.loc[
                    group_top_covariate_index, :
                ]
                ...
                if group_top_covariate == top_covariate:
                    appendix = "-X"
                label = "{}-{}-Group{}{}".format(
                    pic, group_top_covariate, group_index, appendix
                )
                group_top_pic.index = [label]
                top_pics_per_group_summary_stats.append(
                    [
                        label,
                        group_summary_stats_df.loc[group_top_covariate_index, "start"],
                        group_summary_stats_df.loc[group_top_covariate_index, "end"],
                    ]
                )
            ...
            self.plot_pics_df(
                pics_df=pics_df, pic_groups=pic_groups, outdir=pic_plot_dir, name=name
            )
            if previous_pics_df is not None:
                self.plot_compare_top_pics_per_group(
                    current_pics_df=pics_df,
            ...
            tech_covariate_with_inter_df = self.load_file(
                tech_covariate_with_inter_path, header=0, index_col=0
            )
            print(tech_covariate_with_inter_df)
            new_tech_covariate_with_inter_df = tech_covariate_with_inter_df.T.merge(
                top_pic.T, left_index=True, right_index=True
            ).T
            print(new_tech_covariate_with_inter_df)

            print("\tSaving file.")
            new_tech_covariate_with_inter_outpath = os.path.join(
                self.tech_covariate_with_inter_outdir,
                os.path.basename(tech_covariate_with_inter_path).replace(".txt.gz", "")
                + "_{}.txt.gz".format(pic),
            )
            self.save_file(
                df=new_tech_covariate_with_inter_df,
                outpath=new_tech_covariate_with_inter_outpath,
            )

            # Overwriting -tci argument.
            tech_covariate_with_inter_path = new_tech_covariate_with_inter_outpath
            del (
                new_tech_covariate_with_inter_df,
                tech_covariate_with_inter_df,
                new_tech_covariate_with_inter_outpath,
                summary_stats_df,
            )
            print("")
        ...
        print("  Selecting top PIC per group")
        top_pics_per_group_summary_stats_df = pd.DataFrame(
            top_pics_per_group_summary_stats, columns=["label", "start", "end"]
        )
        print(top_pics_per_group_summary_stats_df)
        top_unique_pics_per_group = []
        for group_index, group_indices in pic_groups.items():
            group_summary_stats_df = top_pics_per_group_summary_stats_df.loc[
                top_pics_per_group_summary_stats_df["label"].isin(group_indices), :
            ]
            print(
                "\tGroup{}:\tavg. start: {:.2f}\tavg. end: {:.2f}".format(
                    group_index,
                    group_summary_stats_df["start"].mean(),
                    top_pics_per_group_summary_stats_df["end"].mean(),
                )
            )
            group_top_covariate, _, _ = group_summary_stats_df.loc[
                group_summary_stats_df["start"].idxmax(), :
            ]
        ...
        top_unique_pics_per_group_path = os.path.join(
            self.outdir, "all_found_unique_pics.txt.gz"
        )
        self.save_file(
            df=top_unique_pics_per_group_df, outpath=top_unique_pics_per_group_path
        )
        ...
        if "BIOS" in self.outfolder:
            plot_scripts_dir = (
                "/groups/umcg-bios/tmp01/projects/PICALO/dev/plot_scripts"
            )
            ...
            plot_scripts_dir = (
                "/groups/umcg-biogen/tmp01/output/2020-11-10-PICALO/dev/plot_scripts"
            )
        ...
        command = [
            "python3",
            os.path.join(plot_scripts_dir, "create_correlation_heatmap.py"),
            "-rd",
            top_unique_pics_per_group_path,
            "-rn",
            self.outfolder,
            "-o",
            self.outfolder,
        ...
        command = [
            ...
            "AvgExprCorrelation",
            "-o",
            self.outfolder + "_vs_AvgExprCorrelation",
        ]
        self.run_command(command)
        ...
        command = [
            ...
            "-o",
            self.outfolder + "_vs_RNASeqAlignmentMetrics",
        ]
        self.run_command(command)
        ...
        command = [
            ...
            "cell fractions",
            "-o",
            self.outfolder + "_vs_CellFractions",
        ]
        self.run_command(command)
        ...
        if "BIOS" in self.outfolder:
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_CellCounts.txt.gz",
                "-cn",
                "cell counts",
                "-o",
                self.outfolder + "_vs_CellCounts",
            ]
            self.run_command(command)
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_BloodStats.txt.gz",
                "-cn",
                "blood stats",
                "-o",
                self.outfolder + "_vs_BloodStats",
            ]
            self.run_command(command)
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_picalo_files/BIOS-BIOS-cis-NoRNAPhenoNA-NoSexNA-NoMixups-NoMDSOutlier-NoRNAseqAlignmentMetrics/SP140.txt.gz",
                "-cn",
                "SP140",
                "-o",
                self.outfolder + "_vs_SP140",
            ]
            self.run_command(command)
        ...
        if "MetaBrain" in self.outfolder:
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-biogen/tmp01/output/2019-11-06-FreezeTwoDotOne/2020-10-12-deconvolution/deconvolution/matrix_preparation/2022-01-21-CortexEUR-cis-NegativeToZero-DatasetAndRAMCorrected/perform_deconvolution/deconvolution_table_complete.txt.gz",
                "-cn",
                "cell fractions",
                "-o",
                self.outfolder + "_vs_CellFractionComplete",
            ]
            self.run_command(command)
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-biogen/tmp01/output/2019-11-06-FreezeTwoDotOne/2020-10-12-deconvolution/deconvolution/data/AMP-AD/IHC_counts.txt.gz",
                "-cn",
                "IHC",
                "-o",
                self.outfolder + "_vs_IHC",
            ]
            self.run_command(command)
            ...
            command = [
                ...
                self.outfolder,
                "-cd",
                "/groups/umcg-biogen/tmp01/output/2019-11-06-FreezeTwoDotOne/2020-10-12-deconvolution/deconvolution/data/AMP-AD/single_cell_counts.txt.gz",
                "-cn",
                "SCC",
                "-o",
                self.outfolder + "_vs_SCC",
            ]
            self.run_command(command)
    ...
    def create_job_file(
        ...
        lines = [
            ...
            "#SBATCH --output={}".format(
                os.path.join(jobs_output_dir, job_name + ".out")
            ),
            "#SBATCH --error={}".format(
                os.path.join(jobs_output_dir, job_name + ".out")
            ),
    ...
    def combine_picalo_info_files(self, job_names):
        ...
        for job_name in job_names:
            fpath = os.path.join(
                self.picalo_path, "output", job_name, "PIC1", "info.txt.gz"
            )
            if not os.path.exists(fpath):
                print("{} does not exist".format(fpath))
                info_df = pd.DataFrame(
                    [
                        job_name.split("-")[-1].replace("AsCov", ""),
                        0,
                        np.nan,
                        np.nan,
                        np.nan,
            ...
            summary_stats.append(
                [
                    info_df.loc["iteration0", "covariate"],
        ...
        info_df_m.loc[info_df_m["value"] > 0, "log10 value"] = np.log10(
            info_df_m.loc[info_df_m["value"] > 0, "value"].astype(float)
        )

        # Construct summary stats df.
        summary_stats_df = pd.DataFrame(
            summary_stats, columns=["covariate", "start", "end"], index=job_names
        )
    ...
    def combine_picalo_pics(self, job_names):
        ...
        for job_name in job_names:
            fpath = os.path.join(
                self.picalo_path, "output", job_name, "components.txt.gz"
            )
            ...
            if len(groups) == 0:
                ...
                for group_count, group_indices in groups.items():
                    ...
                    for group_index in group_indices:
                        coef, _ = stats.pearsonr(
                            df.loc[group_index, :], df.loc[index, :]
                        )
    ...
    def plot_info_df_m(self, info_df_m, pic_groups, outdir, name):
        ...
        for group_index, group_indices in pic_groups.items():
            info_df_m.loc[
                info_df_m["covariate"].isin(group_indices), "group"
            ] = group_index
    ...
    def plot_summary_stats(self, summary_stats_df, pic_groups, outdir, name):
        ...
        for group_index, group_indices in pic_groups.items():
            summary_stats_df.loc[
                summary_stats_df["covariate"].isin(group_indices), "group"
            ] = group_index
    ...
    def barplot(
        ...
        g = sns.barplot(x=x, y=y, hue=hue, palette=palette, dodge=False, data=df)

        g.set_title(title, fontsize=14, fontweight="bold")
        g.set_xlabel(xlabel, fontsize=10, fontweight="bold")
        g.set_ylabel(ylabel, fontsize=10, fontweight="bold")
    ...
    def correlate_single(df):
        ...
        for i, index1 in enumerate(df.index):
            for j, index2 in enumerate(df.index):
                corr_data = df.loc[[index1], :].T.merge(
                    df.loc[[index2], :].T, left_index=True, right_index=True
                )
    ...
    @staticmethod
    def plot_clustermap(
        df, colors=None, outdir=None, row_cluster=True, col_cluster=True, filename=""
    ):
    ...
    def plot_compare_top_pics_per_group(
        self, current_pics_df, previous_pics_df, outdir, name
    ):
        if current_pics_df.shape[0] <= 1 and previous_pics_df.shape[0] <= 1:
            return
        corr_df = self.correlate_double(df1=current_pics_df, df2=previous_pics_df)
        self.plot_clustermap(
            df=corr_df,
            row_cluster=False,
            col_cluster=False,
    ...
    def correlate_double(df1, df2):
        ...
        for i, index1 in enumerate(df1.index):
            for j, index2 in enumerate(df2.index):
                corr_data = df1.loc[[index1], :].T.merge(
                    df2.loc[[index2], :].T, left_index=True, right_index=True
                )
                corr_data.dropna(inplace=True)
                coef = np.nan
                if corr_data.std(axis=0).min() > 0:
    ...
    def print_arguments(self):
        ...
        print(
            "  > Technical covariates with interaction: {}".format(
        ...
        print(
            "  > Max end datetime: {}".format(
                datetime.fromtimestamp(self.max_end_time).strftime("%d-%m-%Y, %H:%M:%S")
            )
        )
        print("")
