...
class main:
    ...
    def barplot(
        ...
        g = sns.barplot(x=x, y=y, hue=hue, data=df, color=color, palette=palette, ax=ax)

        y_adjust = ax.get_ylim()[1] * 0.01
        for i, (_, row) in enumerate(df.iterrows()):
            g.text(i, row[y] + y_adjust, round(row[y], 2), color="#000000", ha="center")
