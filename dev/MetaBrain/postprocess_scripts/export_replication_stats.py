...
class main:
    def __init__(self):
        ...
        # Set variables.
        outdir = os.path.join(
            str(Path(__file__).parent.parent), "export_replication_stats"
        )
    ...
    def start(self):
        with pd.ExcelWriter(self.outpath) as writer:
            ...
            replication_stats = pd.pivot_table(
                df.loc[df["label"] == "discovery significant", :],
                values="value",
                index="col",
                columns="variable",
            )
            ...
            replication_stats.to_excel(
                writer, sheet_name="brain replication stats", na_rep="NA", index=False
            )
            print(
                "Saving sheet 'brain replication stats' with shape {}".format(df.shape)
            )
