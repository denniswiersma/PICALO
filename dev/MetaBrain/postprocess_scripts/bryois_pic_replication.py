...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "bryois_pic_replication"
        )
    ...
    def start(self):
        ...
        for i in range(1, 6):
            ...
            discovery_ieqtl_df["SNP"] = discovery_ieqtl_df["SNP"].str.split(
                ":", expand=True
            )[2]
            discovery_ieqtl_df["gene"] = discovery_ieqtl_df["gene"].str.split(
                ".", expand=True
            )[0]
            discovery_ieqtl_df["flip"] = discovery_ieqtl_df["SNP"].map(flip_dict)
            discovery_ieqtl_df.index = (
                discovery_ieqtl_df["gene"] + "_" + discovery_ieqtl_df["SNP"]
            )
            discovery_ieqtl_df = discovery_ieqtl_df[
                ["beta-interaction", "p-value", "FDR", "flip"]
            ]
            ...
            ieqtl_df = discovery_ieqtl_df.merge(
                replication_df, left_index=True, right_index=True, how="inner"
            )
            if ieqtl_df.shape[0] == 0:
                continue

            discovery_mask = (
                ieqtl_df["MetaBrain {} FDR".format(pic)] <= 0.05
            ).to_numpy()
            print("\t  Discovery N-ieqtls: {:,}".format(np.sum(discovery_mask)))
            for repl_ct in replication_cell_types:
                ieqtl_df["Bryois {} beta".format(repl_ct)] = (
                    ieqtl_df["Bryois {} beta".format(repl_ct)] * ieqtl_df["flip"]
                )

                ieqtl_df["Bryois {} FDR".format(repl_ct)] = np.nan
                replication_mask = (
                    ~ieqtl_df["Bryois {} p-value".format(repl_ct)].isna()
                ).to_numpy()
                mask = np.logical_and(discovery_mask, replication_mask)
                n_overlap = np.sum(mask)
                if n_overlap > 1:
                    ieqtl_df.loc[
                        mask, "Bryois {} FDR".format(repl_ct)
                    ] = multitest.multipletests(
                        ieqtl_df.loc[mask, "Bryois {} p-value".format(repl_ct)],
                        method="fdr_bh",
                    )[
                        1
                    ]
                n_replicating = ieqtl_df.loc[
                    ieqtl_df["Bryois {} FDR".format(repl_ct)] <= 0.05, :
                ].shape[0]
                print(
                    "\t  {}{} replication N-ieqtls: {:,} / {:,} [{:.2f}%]".format(
                        repl_ct,
                        (max_repl_ct_length - len(str(repl_ct))) * " ",
                        n_replicating,
                        n_overlap,
            ...
            replication_stats_df = self.plot(
                df=ieqtl_df, cell_types=replication_cell_types, pic=pic
            )
        ...
        print("Visualizing.")
        self.combined_plot(dfs=ieqtl_dfs, pics=pics, cell_types=replication_cell_types)
    ...
    def plot(self, df, cell_types, pic):
        ...
        # Add the main title.
        fig.suptitle(
            "{} ieQTL replication in single-nucleus eQTLs\nBryois et al. 2021".format(
                pic
            ),
            fontsize=40,
            color="#000000",
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir, "bryois_{}_replication_plot.{}".format(pic, extension)
                )
            )
        plt.close()
    ...
    def combined_plot(self, dfs, pics, cell_types):
        ...
        for col_index, pic in enumerate(pics):
            for row_index, ct in enumerate(cell_types):
                ...
                plot_df["facecolors"] = [
                    ...
                    for fdr_value in plot_df["Bryois FDR"]
                ]
        ...
        # Add the main title.
        fig.suptitle(
            "PIC ieQTL replication in single-nucleus eQTLs\nBryois et al. 2021",
            fontsize=40,
            color="#000000",
            weight="bold",
        )
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir, "bryois_PIC_replication_plot.{}".format(extension)
                )
            )
        plt.close()
