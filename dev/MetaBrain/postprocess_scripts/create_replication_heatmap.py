...
class main:
    ...
    def start(self):
        ...
        df = self.load_file(
            os.path.join(self.work_dir, "replication_stats.txt.gz"),
            header=0,
            index_col=0,
        )
        print(df)
    ...
    def plot(self, n_df, stats_df):
        ...
        sns.set(
            rc={
                "figure.figsize": (
                    1 * sum([stats[1].shape[0] for stats in stats_df]) + 10,
                    1 * max([stats[1].shape[1] for stats in stats_df]) + 10,
                )
            }
        )
        sns.set_style("ticks")
        fig, axes = plt.subplots(nrows=1, ncols=ncols, sharex="none", sharey="none")
        sns.set(color_codes=True)

        for i, (label, df, vmin, vmax, center, precision, value_appendix) in enumerate(
            stats_df
        ):
            ax = axes[i]
            ...
            for index in df.index:
                for column in df.columns:
                    annot_df.loc[index, column] = "{:.{}f}{}".format(
                        df.loc[index, column], precision, value_appendix
                    )
            ...
            sns.heatmap(
                ...
                cbar=False,
                annot_kws={"size": 25, "color": "#000000"},
                ax=ax,
            )
            ...
            ax.set_title(label, fontsize=30, color="#000000", weight="bold")
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.work_dir, "replication_stats_heatmap.{}".format(extension)
                )
            )
        plt.close()
