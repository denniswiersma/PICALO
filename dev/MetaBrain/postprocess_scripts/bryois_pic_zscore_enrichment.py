...
class main:
    ...
    def start(self):
        ...
        for suffix in ["p-value", "beta"]:
            repl_subset_df = replication_df.loc[
                :,
                ["index", "effect_allele"]
                + [col for col in replication_df if col.endswith(suffix)],
            ].copy()
            repl_subset_df.columns = [
                col.replace(" {}".format(suffix), "") for col in repl_subset_df.columns
            ]
            repl_subset_df.columns = [
                self.bryois_ct_trans[col] if col in self.bryois_ct_trans else col
                for col in repl_subset_df.columns
            ]
            if suffix == "beta":
                cell_types = repl_subset_df.columns[2:]
                repl_subset_df.loc[:, cell_types] = repl_subset_df.loc[
                    :, cell_types
                ].abs()
                # repl_subset_df.loc[:, cell_types] = repl_subset_df.loc[:, cell_types].subtract(repl_subset_df.loc[:, cell_types].mean(axis=1), axis=0)
                repl_subset_df.loc[:, cell_types] = (
                    repl_subset_df.loc[:, cell_types]
                    - repl_subset_df.loc[:, cell_types].mean(axis=0)
                ) / repl_subset_df.loc[:, cell_types].std(axis=0)
        ...
        replication_df = reduce(
            lambda df1, df2: pd.merge(
                df1, df2, on=["index", "effect_allele", "cell type"]
            ),
            replication_df_list,
        )
        ...
        for inpath in inpaths:
            ...
            discovery_ieqtl_df.index = (
                discovery_ieqtl_df["gene"].str.split(".", expand=True)[0]
                + "_"
                + discovery_ieqtl_df["SNP"].str.split(":", expand=True)[2]
            )

            signif = list(
                discovery_ieqtl_df.loc[discovery_ieqtl_df["FDR"] < 0.05, :].index
            )
            not_signif = list(
                discovery_ieqtl_df.loc[discovery_ieqtl_df["FDR"] >= 0.05, :].index
            )
            ...
            pic_replication_df.loc[
                pic_replication_df["index"].isin(signif), "signif"
            ] = True
            pic_replication_df.loc[
                pic_replication_df["index"].isin(not_signif), "signif"
            ] = False
            ...
            self.plot_single_boxplot(
                ...
                title=filename,
                filename="{}_{}_beta_boxplot".format(self.out_filename, filename),
            )
    ...
    def boxplot(
        ...
        sns.violinplot(
            x=x, y=y, hue=hue, data=df, palette=palette, cut=0, dodge=True, ax=ax
        )

        plt.setp(ax.collections, alpha=0.75)

        sns.boxplot(x=x, y=y, hue=hue, data=df, color="white", dodge=True, ax=ax)
