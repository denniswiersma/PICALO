...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="The column in -gi / --gene_info matrix" "with the ENSG IDs.",
        )
        parser.add_argument(
            "-hgnc",
            "--hgnc_column",
            type=str,
            required=True,
            help="The column in -gi / --gene_info matrix" "with the HGNC names.",
        )
        parser.add_argument(
            "-o",
            "--outfile",
    ...
    def start(self):
        ...
        for i in range(10):
            ...
            #     continue
            component_gc_df = gc_df.loc[
                (gc_df["component"] == component) & (gc_df["gene (HGNC)"].isin(genes)),
                :,
            ].copy()
            ...
            for cell_type in cell_types:
                ...
                overlap = set(component_gc_df.index.tolist()).intersection(
                    set(ct_df.index.tolist())
                )
                print(len(overlap))
                ...
                for i in range(comp_ct_df.shape[1]):
                    coef, p = stats.pearsonr(
                        comp_ct_df.iloc[:, i], comp_ct_gc_df["correlation"]
                    )
                ...
                results.append(
                    (cell_type, mean_coef, np.std(coefficients), abs(mean_coef))
                )
    ...
    def plot_heatmap(self, corr_df):
        ...
        g.savefig(
            os.path.join(
                self.outdir, "{}_SNRNASeq_corr_heatmap.png".format(self.out_filename)
            )
        )
        plt.close()
