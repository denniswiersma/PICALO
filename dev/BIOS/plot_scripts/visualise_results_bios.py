...
class main:
    ...
    def start(self):
        ...
        command = [
            "python3",
            "covariate_selection_lineplot.py",
            "-i",
            self.input_data_path,
            "-p",
            self.palette_path,
        ...
        command = [
            ...
            self.input_data_path,
            "-e",
            os.path.join(self.pf_path, "eQTLProbesFDR0.05-ProbeLevel-Available.txt.gz"),
            "-p",
            self.palette_path,
            "-o",
            self.outname,
        ...
        for i in range(1, 50):
            ...
            if os.path.exists(comp_iterations_path):
                pics.append(pic)
                fpaths = glob.glob(
                    os.path.join(self.input_data_path, pic, "results_iteration*")
                )
                fpaths.sort()
        ...
        if not os.path.exists(components_path):
            ...
            if len(data) > 0:
                ...
                self.save_file(
                    components_df.T, outpath=components_path, header=True, index=True
                )
        ...
        for pic in pics:
            command = [
                ...
                pic,
                "-yd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/correlate_samples_with_avg_gene_expression/BIOS_CorrelationsWithAverageExpression.txt.gz",
                "-y_transpose",
                "-yi",
                "AvgExprCorrelation",
                "-std",
        ...
        for i in range(1, 6):
            pic = "PIC{}".format(i)
            fpath = os.path.join(
                self.input_data_path, "PIC_interactions", "{}.txt.gz".format(pic)
            )
        ...
        command = [
            ...
            components_path,
            "-std",
            "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_sex.txt.gz",
            "-n",
            "5",
            "-p",
            self.palette_path,
        ...
        if os.path.exists(components_path):
            ...
            command = [
                ...
                self.outname,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_RNA_AlignmentMetrics.txt.gz",
                "-cn",
            ...
            command = [
                ...
                os.path.join(
                    ...
                    "data",
                    "gene_read_counts_BIOS_and_LLD_passQC.tsv.SampleSelection.ProbesWithZeroVarianceRemoved.TMM.SampleSelection.ProbesWithZeroVarianceRemoved.Log2Transformed.PCAOverSamplesEigenvectors.txt.gz",
                ),
                "-cn",
                "PCA before cov. corr.",
                "-o",
            ...
            command = [
                ...
                os.path.join(
                    self.expression_preprocessing_path,
                    "data",
                    "gene_read_counts_BIOS_and_LLD_passQC.tsv.SampleSelection.ProbesWithZeroVarianceRemoved.TMM.SampleSelection.ProbesWithZeroVarianceRemoved.Log2Transformed.CovariatesRemovedOLS.ScaleAndLocReturned.PCAOverSamplesEigenvectors.txt.gz",
                ),
                "-cn",
                "PCA after cov. corr.",
                "-o",
            ...
            command = [
                ...
                os.path.join(
                    self.expression_preprocessing_path,
                    "data",
                    "gene_read_counts_BIOS_and_LLD_passQC.tsv.SampleSelection.ProbesWithZeroVarianceRemoved.TMM.SampleSelection.ProbesWithZeroVarianceRemoved.Log2Transformed.ProbesCentered.SamplesZTransformed.PCAOverSamplesEigenvectors.txt.gz",
                ),
                "-cn",
            ...
            command = [
                ...
                self.outname,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_CellFractionPercentages.txt.gz",
                "-cn",
                "cell fractions %",
                "-o",
                self.outname + "_vs_CellFractionPercentages",
            ]
            self.run_command(command)
            ...
            command = [
                ...
                self.outname,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/data/BIOS_cell_types_DeconCell_2019-03-08.txt.gz",
                "-cn",
                "Decon-Cell cell fractions",
                "-o",
                self.outname + "_vs_decon",
                "-e",
            ] + self.extensions
            self.run_command(command)
            ...
            command = [
                ...
                self.outname,
                "-cd",
                "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_phenotype_matrix/BIOS_phenotypes.txt.gz",
                "-cn",
                "phenotypes",
                "-o",
