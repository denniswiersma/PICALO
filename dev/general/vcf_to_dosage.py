...
class main:
    def __init__(self):
        ...
        self.output = os.path.join(
            outdir, os.path.basename(self.input).replace(".vcf.gz", ".tsv.gz")
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-i", "--input", type=str, default=None, help="The input VCF file."
        )
        parser.add_argument(
            "-o",
            "--output",
    ...
    def start(self):
        ...
        for i, line in enumerate(fh):
            ...
            if line.startswith("#CHROM"):
                ...
                fho.write(
                    "\t".join(["SNP", "Alleles", "AltAllele"] + values[samples_index:])
                    + "\n"
                )
                continue
            ...
            for value in values[samples_index:]:
                ...
                if gt_value == "./.":
                    ...
                    dosages.append(
                        str(sum([float(dosage) for dosage in gt_value.split("/")]))
                    )
                elif "|" in gt_value:
                    dosages.append(
                        str(sum([float(dosage) for dosage in gt_value.split("|")]))
