...
class main:
    def __init__(self):
        ...
        # Set variables.
        outdir = os.path.join(
            str(Path(__file__).parent.parent), "export_toppfun_to_excel"
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-b", "--bios", type=str, required=True, help="The path to BIOS file."
        )
        parser.add_argument(
            "-m",
            "--metabrain",
    ...
    def start(self):
        ...
        for df in [bios_df, metabrain_df]:
            ...
            df.drop(
                ["QValueFDRBY", "QValueBonferroni", "Genes", "covariate", "N"],
                axis=1,
                inplace=True,
            )
            df.columns = [
                col_trans_dict[col] if col in col_trans_dict else col
            ...
            for df, name in [(bios_df, "blood"), (metabrain_df, "brain")]:
                for category in ["ToppCell", "Pathway"]:
                    for subset in ["genes", "ieQTL genes"]:
                        ...
                        susbet_df.to_excel(
                            writer, sheet_name=sheet_name, na_rep="NA", index=False
                        )
                        print(
                            "Saving sheet '{}' with shape {}".format(
                                sheet_name, susbet_df.shape
                            )
                        )
                        print("")
