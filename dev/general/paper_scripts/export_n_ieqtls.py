...
class main:
    def __init__(self):
        ...
        self.blood_indir = (
            "/groups/umcg-bios/tmp01/projects/PICALO/fast_interaction_mapper"
        )
    ...
    def start(self):
        with pd.ExcelWriter(self.outpath) as writer:
            for indir, filename, symbol, name in [
                (self.blood_indir, self.blood_filename, "-", "blood"),
                (self.brain_indir, self.brain_filename, "_", "brain"),
            ]:
                print("Loading {} data".format(name))

                pic_df_default = self.load_data(
                    indir=os.path.join(indir, filename + symbol + "PICsAsCov")
                )
                pic_df_cond = self.load_data(
                    indir=os.path.join(
                        indir, filename + symbol + "PICsAsCov-Conditional"
                    ),
                    conditional=True,
                )

                pc_df_default = self.load_data(
                    indir=os.path.join(indir, filename + symbol + "PCsAsCov")
                )
                pc_df_cond = self.load_data(
                    indir=os.path.join(
                        indir, filename + symbol + "PCsAsCov-Conditional"
                    ),
                    conditional=True,
                )

                df = pd.concat(
                    [pic_df_default, pic_df_cond, pc_df_default, pc_df_cond], axis=1
                )
                df.columns = [
                    "PIC - default",
                    "PIC - conditional",
