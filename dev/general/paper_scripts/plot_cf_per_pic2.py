...
class main:
    def __init__(self):
        ...
        self.palette = {"Myeloid Progenitor": "#D55E00", "Neuron": "#0072B2"}
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-pi", "--pic_index", type=str, default=None, help="The PIC to plot."
        )
        parser.add_argument(
            "-o",
            "--outfile",
    ...
    def start(self):
        ...
        df = cf_df[[self.cell_type]].merge(
            pics_df[[self.pic]], left_index=True, right_index=True
        )
        df.columns = ["proportion", "loading"]
        df["proportion"] = df["proportion"] * 100

        # Normalize loadings between bins.
        df["normalized"] = self.normalize(
            values=df["loading"],
    ...
    def plot_boxplot(
        ...
        fig, (ax1, ax2) = plt.subplots(
            nrows=2, ncols=1, gridspec_kw={"height_ratios": [0.1, 0.9]}
        )
        ...
        handles = [
            mpatches.Patch(color=color, label=self.cell_type),
        ...
        sns.barplot(x=x, y=y, order=order, data=df, color=color, ax=ax2)
