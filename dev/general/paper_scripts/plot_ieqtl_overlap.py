...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "plot_ieqtl_overlap"
        )
    ...
    def start(self):
        ...
        x_data, x_labels = self.load_data(
            indir=self.x_input_directory, conditional=self.x_conditional
        )
        y_data, y_labels = self.load_data(
            indir=self.y_input_directory, conditional=self.y_conditional
        )

        print("Create overlap.")
        df, annot_df = self.get_overlap(
            x_data=x_data, x_labels=x_labels, y_data=y_data, y_labels=y_labels
        )

        print("Plotting.")
        self.plot_heatmap(
            df=df, annot_df=annot_df, xlabel=self.x_label, ylabel=self.y_label
        )
    ...
    def plot_heatmap(
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir,
                    "{}{}.{}".format(self.out_filename, appendix, extension),
                )
            )
        plt.close()
