...
class main:
    def __init__(self):
        ...
        self.file_outdir = os.path.join(
            base_dir, "plot_picalo_with_multiple_start_pos", self.filename
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-f", "--filename", type=str, required=True, help="The PICALO filename."
        )
        parser.add_argument(
            "-rx",
            "--regplot_x",
    ...
    def start(self):
        ...
        for pic_index in range(1, self.n_pics + 1):
            for comp_index in range(1, self.n_comps + 1):
                ...
                iteration_fpath = os.path.join(
                    self.indir,
                    "{}-{}AsCov".format(self.filename, index),
                    "PIC1",
                    "iteration.txt.gz",
                )
                ...
                result_files = glob.glob(
                    os.path.join(
                        ...
                        "{}-{}AsCov".format(self.filename, index),
                        "PIC1",
                        "results_iteration*.txt.gz",
                    )
                )
                result_files.sort(key=self.natural_keys)

                first_result_df = self.load_file(
                    result_files[0], header=0, index_col=None
                )
                first_result_df.index = (
                    first_result_df["SNP"] + "_" + first_result_df["gene"]
                )
                first_ieqtls[index] = set(
                    first_result_df.loc[first_result_df["FDR"] <= 0.05, :].index
                )

                last_result_df = self.load_file(
                    result_files[-1], header=0, index_col=None
                )
                last_result_df.index = (
                    last_result_df["SNP"] + "_" + last_result_df["gene"]
                )
                last_ieqtls[index] = set(
                    last_result_df.loc[last_result_df["FDR"] <= 0.05, :].index
                )
        ...
        print("Overlap.")
        first_overlap_df, first_annot_df = self.overlap(
            ieqtls=first_ieqtls, indices=indices
        )
        last_overlap_df, last_annot_df = self.overlap(
            ieqtls=last_ieqtls, indices=indices
        )
        ...
        print("Saving output.")
        for tmp_df, suffix in (
            [iteration_df, "iteration_data"],
            ...
            self.save_file(
                ...
                outpath=os.path.join(
                    self.file_outdir, "{}_{}.txt.gz".format(self.filename, suffix)
                ),
            )
        # exit()
        ...
        self.plot_heatmap(
            heatmap_df=first_overlap_df,
            annot_df=first_annot_df,
            data_type="first_overlap",
        )
        self.plot_heatmap(
            heatmap_df=last_overlap_df, annot_df=last_annot_df, data_type="last_overlap"
        )
    ...
    def plot_heatmap(
        self, heatmap_df, annot_df=None, reg_df=None, reg_x="x", reg_y="y", data_type=""
    ):
        fontsize = 16
        if annot_df is None:
            fontsize = 22
        ...
        fig, axes = plt.subplots(
            ...
            sharex="none",
            sharey="none",
            figsize=(1 * heatmap_df.shape[1] + 10, 1 * heatmap_df.shape[0] + 10),
        )
        ...
        for row_index, row_pic_index in enumerate(range(1, self.n_pics + 1)):
            ...
            for col_index, col_pic_index in enumerate(range(1, self.n_pics + 1)):
                ...
                if (
                    (row_index == 0)
                    and (col_index == (self.n_pics - 1))
                    and reg_df is not None
                ):
                    ...
                    sns.regplot(
                        ...
                        line_kws={"color": "#b22222", "linewidth": 5},
                        ax=ax,
                    )
                    ...
                    ax.set_xlabel(reg_x, fontsize=40, fontweight="bold")
                    ax.set_ylabel(reg_y, fontsize=40, fontweight="bold")
                    continue
        ...
        for extension in self.extensions:
            filename = "{}_picalo_multiple_start_pos_{}_heatmap.{}".format(
                self.filename, data_type, extension
            )
