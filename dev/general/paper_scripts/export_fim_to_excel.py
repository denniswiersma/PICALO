...
class main:
    ...
    def start(self):
        ...
        alleles_df["Affect Allele"] = alleles_df["Alleles"].str.split(
            "/", n=1, expand=True
        )[1]
        ...
        gene_info_df["gene"] = gene_info_df["ArrayAddress"].str.split(
            ".", n=1, expand=True
        )[0]
        ...
        with pd.ExcelWriter(self.outpath) as writer:
            ...
            genotype_stats_df = self.load_file(
                os.path.join(self.indir, "genotype_stats.txt.gz"),
                header=0,
                index_col=None,
            )
            ...
            for i in range(1, 100):
                ...
                pic_path = os.path.join(
                    self.indir,
                    "{}{}.txt.gz".format(
                        pic, "_conditional" if self.conditional else ""
                    ),
                )
                print(pic_path)
                ...
                pic_df.columns = [
                    col if col != "FDR" else "BH-FDR" for col in pic_df.columns
                ]
