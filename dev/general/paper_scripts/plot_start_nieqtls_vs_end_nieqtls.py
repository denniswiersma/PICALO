    ...
    def load_data(self, indir, filename):
        ...
        for pic_index in range(1, 6):
            for comp_index in range(1, 26):
                ...
                fpath = os.path.join(
                    indir, "{}-{}AsCov".format(filename, index), "PIC1", "info.txt.gz"
                )
    ...
    def plot_regplot(
        ...
        for extension in self.extensions:
            outpath = os.path.join(
                self.outdir,
                "{}_start_nieqtls_vs_end_nieqtls.{}".format(self.outfile, extension),
            )
            fig.savefig(outpath)
        plt.close()

    @staticmethod
    def plot_single_regplot(
        fig,
        ax,
        df,
        x="x",
        ...
        handles = [
            mpatches.Patch(color="#000000", label="ALL [r={:.2f}]".format(pearson_coef))
        ]
        if hue is not None:
