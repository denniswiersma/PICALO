...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "plot_interferon_signaling"
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-d", "--data", type=str, required=True, help="The path to the data."
        )
        parser.add_argument(
            "-p",
            "--pics",
            ...
            type=str,
            default=all,
            help="The PICs to analyse." "Default: 'png'.",
        )
        parser.add_argument(
            "-fc",
            "--fold_change",
    ...
    def start(self):
        ...
        print("Loading data.")
        experiment_data_df = self.load_file(
            self.experiment_data_path, header=0, index_col=None, skiprows=18
        )
        ...
        experiment_data_df = experiment_data_df.loc[
            experiment_data_df["Fold Change"].abs() > self.fold_change, :
        ]
        interferon_annotation_df = (
            experiment_data_df.groupby("Ensembl ID")["Inteferome Type"]
            .apply(lambda x: ",".join([value for value in set(x)]))
            .reset_index()
        )
        print(interferon_annotation_df)

        print("Merging data.")
        df = picalo_data_df.merge(
            interferon_annotation_df,
            left_on="ProbeName",
            right_on="Ensembl ID",
            how="left",
        ...
        for pic in self.pics:
            ...
            for correlation_direction in ["positive", "negative"]:
                ...
                if corr_subset_df.shape[0] > 0:
                    ...
                    is_subset_counts = (
                        corr_subset_df["Inteferome Type"].value_counts().to_frame()
                    )
                    is_subset_counts.columns = ["subset"]
                    is_counts = is_total_counts.merge(
                        is_subset_counts, left_index=True, right_index=True
                    )
                    print(is_counts)
                    ...
                    for unique in corr_subset_df["Inteferome Type"].unique():
                        print(
                            " ".join(
                                corr_subset_df.loc[
                                    corr_subset_df["Inteferome Type"] == unique,
                                    "symbol",
                                ].values.tolist()
                            )
                        )
                ...
                if signif_subset_df.shape[0] > 0:
                    ...
                    is_subset_counts = (
                        signif_subset_df["Inteferome Type"].value_counts().to_frame()
                    )
                    is_subset_counts.columns = ["subset"]
                    is_counts = is_total_counts.merge(
                        is_subset_counts, left_index=True, right_index=True
                        ...
                        print(
                            " ".join(
                                signif_subset_df.loc[
                                    signif_subset_df["Inteferome Type"] == unique,
                                    "symbol",
                                ].values.tolist()
                            )
                        )
    ...
    def print_arguments(self):
        print("Arguments:")
        print(
            "  > Interfome experiment data path: {}".format(self.experiment_data_path)
        )
