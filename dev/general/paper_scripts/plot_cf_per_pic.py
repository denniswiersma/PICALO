...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            "-center", action="store_true", help="Center cell fractions."
        )
        parser.add_argument(
            "-o",
            "--outfile",
    ...
    def start(self):
        ...
        cc_dfm = cf_df.melt(
            id_vars=["index"], var_name="cell type", value_name="proportion"
        )
        ...
        for pic in self.selection:
            ...
            for x in range(self.n_bins):
                ...
                df.loc[
                    (df["loading"] > lower_threshold)
                    & (df["loading"] <= upper_threshold),
                    "bin",
                ] = x
    ...
    def create_colormap(self):
        ...
        colors = [
            "#" + "".join(f"{int(i * 255):02X}" for i in x)
            for x in sns.diverging_palette(240, 10, n=self.n_bins + 1)
        ]
