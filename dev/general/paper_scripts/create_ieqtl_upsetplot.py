...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "create_ieqtl_upsetplot"
        )
    ...
    def start(self):
        ...
        ieqtl_data = self.load_data(
            indir=self.input_directory, conditional=self.conditional
        )
        ...
        for extension in self.extensions:
            plt.savefig(
                os.path.join(
                    self.outdir, "{}_upsetplot.{}".format(self.out_filename, extension)
                )
            )
        plt.close()
