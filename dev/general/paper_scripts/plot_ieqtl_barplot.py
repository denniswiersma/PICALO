...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            default=[],
            help="The covariates to exclude from the first" "indir.",
        )
        parser.add_argument(
            "-n1",
            "--n_files1",
            type=int,
            default=None,
            help="The number of files to load from the first" "indir. Default: all.",
        )
        parser.add_argument(
            "-i2",
            "--indir2",
        ...
        parser.add_argument(
            ...
            type=str,
            default=[],
            help="The covariates to exclude from the second" "indir.",
        )
        parser.add_argument(
            "-n2",
            "--n_files2",
            type=int,
            default=None,
            help="The number of files to load from the second" "indir. Default: all.",
        )
        parser.add_argument(
    ...
    def start(self):
        ...
        print("Loading data.")
        df1 = self.load_data(
            indir=self.indir1, exclude=self.exclude1, n_files=self.n_files1
        )

        df2 = self.load_data(
            indir=self.indir2, exclude=self.exclude2, n_files=self.n_files2
        )
    ...
    def load_data(self, indir, exclude, n_files):
        ...
        for i, inpath in enumerate(inpaths):
            ...
            if filename in ["call_rate", "genotype_stats"] or (
                exclude is not None and filename in exclude
            ):
                continue
    ...
    def barplot(
        ...
        g = sns.barplot(x=x, y=y, hue=hue, data=df, palette=palette, ax=ax)
