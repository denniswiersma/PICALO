...
class main:
    ...
    def start(self):
        ...
        badly_imputated_indices = [
            np.round(x, 2)
            for x in np.hstack(
                (np.arange(0.26, 0.75, 0.01), np.arange(1.26, 1.75, 0.01))
            )
        ]
        ...
        for i in range(10):
            ...
            for j in range(100):
                ...
                fpath = os.path.join(
                    self.indir, component, "results_{}.txt.gz".format(iteration)
                )
                if os.path.exists(fpath):
                    ...
                    geno_counts.loc["not imputed", :] = (
                        geno_counts.loc[0, :]
                        + geno_counts.loc[1, :]
                        + geno_counts.loc[2, :]
                    )
                    geno_counts.loc["imputed", :] = (
                        geno_counts.loc["sum", :]
                        - geno_counts.loc["not imputed", :]
                        - geno_counts.loc["missing", :]
                    )
                    ...
                    geno_counts.loc["badly imputed", :] = geno_counts.loc[
                        subset_badly_imputated_indices, :
                    ].sum(axis=0)

                    save_df = geno_counts.loc[
                        ["sum", "missing", "not imputed", "imputed", "badly imputed"], :
                    ].T
                    ...
                    outpath = os.path.join(
                        self.outdir,
                        "{}_genotype_info_{}.txt.gz".format(component, iteration),
                    )
                    self.save_file(save_df, outpath=outpath)
