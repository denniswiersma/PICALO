...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "plot_avg_expression"
        )
    ...
    def start(self):
        ...
        meta_pic_df = self.merge_data_frames(
            interaction_df=meta_pic_data, expression_df=mean_meta_expression_df
        )
        meta_pc_df = self.merge_data_frames(
            interaction_df=meta_pc_data, expression_df=mean_meta_expression_df
        )

        mean_bios_expression_df = bios_expression.mean(axis=1).to_frame()
        mean_bios_expression_df.columns = ["expression"]

        bios_pic_df = self.merge_data_frames(
            interaction_df=bios_pic_data, expression_df=mean_bios_expression_df
        )
        bios_pc_df = self.merge_data_frames(
            interaction_df=bios_pc_data, expression_df=mean_bios_expression_df
        )

        plot_data = []
        for tissue, data, df, tech_cols in (
            ("blood", "PIC", bios_pic_df, self.bios_technical),
            ("blood", "PC", bios_pc_df, self.bios_technical),
            ("brain", "PIC", meta_pic_df, self.meta_technical),
            ("brain", "PC", meta_pc_df, self.meta_technical),
        ):
            bio_cols = [
                col for col in df.columns if col not in tech_cols and col != "gene"
            ]
            ...
            for variable in [
                ...
                if variable == "no interaction":
                    expression = df.loc[
                        df.iloc[:, :-2].sum(axis=1) == 0, "expression"
                    ].tolist()
                    ...
                    expression = df.loc[
                        (df[tech_cols].sum(axis=1) == 0)
                        & (df[bio_cols].sum(axis=1) > 0),
                        "expression",
                    ].tolist()
    ...
    def merge_data_frames(interaction_df, expression_df):
        ...
        interaction_df = interaction_df.merge(
            expression_df, left_on="gene", right_index=True, how="left"
        )
