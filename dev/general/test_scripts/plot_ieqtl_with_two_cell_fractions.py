...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="The path to the original cell_fractions " "matrix",
        )
        parser.add_argument(
            "-ocf",
            ...
            type=str,
            required=True,
            help="The path to the optimized cell_fractions " "matrix",
        )
        parser.add_argument(
            "-d",
            "--decon",
    ...
    def create_color_map(colormap):
        ...
        palette = (
            list(Color(colormap["major"]).range_to(Color(major_small), 50))
    ...
    def start(self):
        ...
        self.plot_eqtls(
            eqtl_df, geno_df, alleles_df, expr_df, cf_df, ocf_df, decon_fdr_df
        )
    ...
    def plot_eqtls(
        self, eqtl_df, geno_df, alleles_df, expr_df, cf_df, ocf_df, decon_df
    ):
        ...
        for i, (index, row) in enumerate(eqtl_df.iterrows()):
            ...
            if not interaction_effect.name.startswith(
                probe_name
            ) or not interaction_effect.name.endswith(snp_name):
            ...
            interaction_effect = interaction_effect.reindex(
                interaction_effect["FDR"].abs().sort_values(ascending=True).index
            )

            if (self.interest is not None and hgnc_name not in self.interest) or (
                interaction_effect.shape[0] == 0
            ):
                continue
            ...
            data = data.loc[(data["genotype"] >= 0.0) & (data["genotype"] <= 2.0), :]
            ...
            if len(interaction_effect.index) > 0:
                eqtl_interaction_outdir = os.path.join(
                    self.outdir,
                ...
                for cov_name, (fdr,) in interaction_effect.iterrows():
                    ...
                    eqtl_data = eqtl_data.merge(
                        cov_data, left_index=True, right_index=True
                    )

                    self.plot_inter_eqtl(
                        df=eqtl_data,
                        ax=ax1,
                    ...
                    for extension in self.extensions:
                        filename = "{}_inter_eqtl_{}_{}_{}_{}.{}".format(
                            i, snp_name, probe_name, hgnc_name, cov_name, extension
                        )
                        print("\t\tSaving plot: {}".format(filename))
                        fig.savefig(
                            os.path.join(eqtl_interaction_outdir, filename), dpi=300
                        )
                    plt.close()
            else:
