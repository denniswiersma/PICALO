...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="The path to the first PIC - gene " "correlation matrix.",
        )
        parser.add_argument(
            "-n1",
            "--name1",
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="The path to the second PIC - gene " "correlation matrix.",
        )
        parser.add_argument(
            "-n2",
            "--name2",
    ...
    def start(self):
        ...
        for i in range(100):
            ...
            df1_subset.columns = [
                "{} avgExpression".format(self.name1),
                "{} correlation".format(self.name1),
            ]
            ...
            df2_subset.columns = [
                "{} avgExpression".format(self.name2),
                "{} correlation".format(self.name2),
            ]

            df = df1_subset.merge(
                df2_subset, how="outer", left_index=True, right_index=True
            )
            print(df)
            del df1_subset, df2_subset

            nas = np.logical_or(
                df["{} correlation".format(self.name1)].isnull(),
                df["{} correlation".format(self.name2)].isnull(),
            )

            coef, _ = stats.pearsonr(
                df.loc[~nas, "{} correlation".format(self.name1)],
                df.loc[~nas, "{} correlation".format(self.name2)],
            )
            print("\t  Correlation: {:.2f}".format(coef))

            if coef < 0:
                df["{} correlation".format(self.name2)] = (
                    df["{} correlation".format(self.name2)] * -1
                )
            ...
            for prefix, set1, set2 in (
                ["top", df1_top_200, df2_top_200],
                ...
                for name, genes in (
                    ...
                    print(
                        "\t\t\t{}:\tavgExpression: {:.2f}\tcorrelation: {:.2f}".format(
                            self.name1,
