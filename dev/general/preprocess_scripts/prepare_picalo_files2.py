...
class main:
    ...
    def martijn():
        ...
        eqtl_df = pd.read_csv(
            "output/picalo_preparer/prep_eqtl/MetaBrain_eQTLProbesFDR0.05-ProbeLevel_GT1.0AvgExprFilter.txt.gz",
            sep="\t",
            header=0,
            index_col=0,
        )
        eqtl_variants = set()
        for snp_name in eqtl_df["SNPName"]:
            chrom, pos, _, alleles = snp_name.split(":")
            allele_a, allel_b = alleles.split("_")
            alleles = sorted([allele_a, allel_b])
            eqtl_variants.add(f"{chrom}:{pos}:{alleles[0]}_{alleles[1]}")
        len(eqtl_variants)  # 11125

        geno_df = pd.read_csv(
            "output/picalo_preparer/filtered_combined_matrices_with_head_new.tsv.gz",
            sep="\t",
            header=0,
            index_col=None,
        )
        geno_variants = set()
        for snp_name in geno_df["SNP"]:
            chrom, pos, allele_a, allel_b = snp_name.split(":")
            alleles = sorted([allele_a, allel_b])
            geno_variants.add(f"{chrom}:{pos}:{alleles[0]}_{alleles[1]}")
        len(geno_variants)  # 11149
        len(eqtl_variants.intersection(geno_variants))  # 11125

        geno_variants.difference(
            eqtl_variants
        )  # varianten in je genotype matrix die je niet had hoeven queryen? Waarom zitten deze erin?

    def start(self):
        def reformat_snpname(snp):
            parts = snp.split(":")
            # Exclude the rsID (third element)
            chrom, pos, allele_info = parts[0], parts[1], parts[3]
            # Replace underscore with colon in allele information
            allele_formatted = allele_info.replace("_", ":")
            # Reconstruct the SNPName
            return f"{chrom}:{pos}:{allele_formatted}"

        def sort_alleles_eqtl(name):
            chr, pos, _, alleles = name.split(":")
            allele_a, allele_b = alleles.split("_")
            alleles = sorted([allele_a, allele_b])
            return f"{chr}:{pos}:{alleles[0]}_{alleles[1]}"

        def sort_alleles_geno(name):
            chr, pos, allele_a, allele_b = name.split(":")
            alleles = sorted([allele_a, allele_b])
            return f"{chr}:{pos}:{alleles[0]}_{alleles[1]}"
        ...
        eqtl_df["SNPName"] = eqtl_df["SNPName"].apply(sort_alleles_eqtl)
        # eqtl_df["SNPName"] = eqtl_df["SNPName"].apply(reformat_snpname)

        print("Preparing genotype and expression file")
        geno_df = self.load_file(self.geno_path, header=0, index_col=0)
        # geno_df["SNP"] = geno_df["SNP"].apply(sort_alleles_geno)
        geno_df.index = geno_df.index.map(sort_alleles_geno)
        ...
        self.save_file(
            df=dataset_df,
            outpath=os.path.join(self.outdir, "datasets_table.txt.gz"),
        )
        self.save_file(
            df=present_eqtl_df,
