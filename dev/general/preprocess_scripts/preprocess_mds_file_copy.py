...
class main:
    ...
    def start(self):
        ...
        nworks = 0
        nfailed = 0
        for i, genotype_id in enumerate(df.index):
            try:
                new_value = gte_dict[genotype_id]
                new_index.append(new_value)
                nworks += 1
            except KeyError:
                nfailed += 1
                # WARNING: probably shouldn't drop these, but temporary fix
                df = df.drop(genotype_id)
        print(f"{nworks} worked and {nfailed} failed")
