...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument("-split_eqtls", action="store_true", help="")
        parser.add_argument("-split_samples", action="store_true", help="")
        parser.add_argument(
            "-od",
            "--outdir",
            type=str,
    ...
    def start(self):
        ...
        std_df = self.load_file(
            os.path.join(self.input_dir, "sample_to_dataset.txt.gz"),
            header=0,
            index_col=None,
        )
        dataset_df = self.load_file(
            os.path.join(self.input_dir, "datasets_table.txt.gz"), header=0, index_col=0
        )
        eqtl_df = self.load_file(
            os.path.join(
                self.input_dir, "eQTLProbesFDR0.05-ProbeLevel-Available.txt.gz"
            ),
            header=0,
            index_col=None,
        )
        geno_df = self.load_file(
            os.path.join(self.input_dir, "genotype_table.txt.gz"), header=0, index_col=0
        )
        allele_df = self.load_file(
            os.path.join(self.input_dir, "genotype_alleles_table.txt.gz"),
            header=0,
            index_col=0,
        )
        expr_df = self.load_file(
            os.path.join(self.input_dir, "expression_table.txt.gz"),
            header=0,
            index_col=0,
        )
        pcpc_df = self.load_file(
            os.path.join(self.input_dir, "first100ExpressionPCs.txt.gz"),
            header=0,
            index_col=0,
        )
        sex_df = self.load_file(
            os.path.join(self.input_dir, "sex_table.txt.gz"), header=0, index_col=0
        )
        mds_df = self.load_file(
            os.path.join(self.input_dir, "mds_table.txt.gz"), header=0, index_col=0
        )
        correction_df = self.load_file(
            os.path.join(self.input_dir, "tech_covariates_with_interaction_df.txt.gz"),
            header=0,
            index_col=0,
        )
        ...
        if self.split_eqtls:
            ...
            print(
                "\tFound {:,} eQTLs on the even chromosomes [{:.2f}%].".format(
                    np.sum(even_mask), (100 / eqtl_df.shape[0]) * np.sum(even_mask)
                )
            )
            print(
                "\tFound {:,} eQTLs on the odd chromosomes [{:.2f}%].".format(
                    np.sum(odd_mask), (100 / eqtl_df.shape[0]) * np.sum(odd_mask)
                )
            )
            print("")
            ...
            for mask, label in ((even_mask, "even"), (odd_mask, "odd")):
                ...
                self.save_file(
                    df=std_df,
                    outpath=os.path.join(subset_outdir, "sample_to_dataset.txt.gz"),
                    index=False,
                )
                self.save_file(
                    df=dataset_df,
                    outpath=os.path.join(subset_outdir, "datasets_table.txt.gz"),
                )
                self.save_file(
                    df=eqtl_df.loc[mask, :],
                    outpath=os.path.join(
                        subset_outdir, "eQTLProbesFDR0.05-ProbeLevel-Available.txt.gz"
                    ),
                    index=False,
                )
                self.save_file(
                    df=geno_df.loc[mask, :],
                    outpath=os.path.join(subset_outdir, "genotype_table.txt.gz"),
                )
                self.save_file(
                    df=allele_df.loc[mask, :],
                    outpath=os.path.join(
                        subset_outdir, "genotype_alleles_table.txt.gz"
                    ),
                )
                self.save_file(
                    df=expr_df.loc[mask, :],
                    outpath=os.path.join(subset_outdir, "expression_table.txt.gz"),
                )
                for n_pcs in range(5, 105, 5):
                    if n_pcs <= pcpc_df.shape[0]:
                        self.save_file(
                            df=pcpc_df.iloc[:n_pcs, :],
                            outpath=os.path.join(
                                subset_outdir,
                                "first{}ExpressionPCs.txt.gz".format(n_pcs),
                            ),
                        )
                self.save_file(
                    df=sex_df, outpath=os.path.join(subset_outdir, "sex_table.txt.gz")
                )
                self.save_file(
                    df=mds_df, outpath=os.path.join(subset_outdir, "mds_table.txt.gz")
                )
                self.save_file(
                    df=correction_df,
                    outpath=os.path.join(
                        subset_outdir, "tech_covariates_with_interaction_df.txt.gz"
                    ),
                )
                print("")
        ...
        if self.split_samples:
            ...
            for n_samples in range(min_n, max_n, self.step):
                ...
                dss_df_copy["subset"] = self.round_series_retain_integer_sum(
                    xs=(dss_df_copy["frac"] * n_samples).to_numpy(), N=n_samples
                )
                if dss_df_copy["subset"].min() < 30:
                    ...
                    dss_df_copy.loc[~mask, "frac"] = (
                        dss_df_copy.loc[~mask, "N"] / dss_df_copy.loc[~mask, "N"].sum()
                    )
                    dss_df_copy.loc[
                        ~mask, "subset"
                    ] = self.round_series_retain_integer_sum(
                        (dss_df_copy.loc[~mask, "frac"] * n_remaining).to_numpy(),
                        N=n_remaining,
                    )
                ...
                for n_pcs in range(5, 105, 5):
                    if n_pcs <= pcpc_df.shape[0]:
                        self.save_file(
                            df=pcpc_df.iloc[:n_pcs, :].loc[:, mask],
                            outpath=os.path.join(
                                subset_outdir,
                                "first{}ExpressionPCs.txt.gz".format(n_pcs),
                            ),
                ...
                self.save_file(
                    df=sex_df.loc[:, mask],
                    outpath=os.path.join(subset_outdir, "sex_table.txt.gz"),
                )
                self.save_file(
                    df=mds_df.loc[:, mask],
                    outpath=os.path.join(subset_outdir, "mds_table.txt.gz"),
                )
                self.save_file(
                    df=correction_df.loc[:, mask],
                    outpath=os.path.join(
                        subset_outdir, "tech_covariates_with_interaction_df.txt.gz"
                    ),
                )
