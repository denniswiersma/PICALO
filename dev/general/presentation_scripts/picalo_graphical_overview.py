...
class main:
    def __init__(self):
        self.n_points = 500
        self.color_map = {0.0: "#E69F00", 1.0: "#0072B2", 2.0: "#D55E00"}
        self.palette = {"ieQTL1": "#E69F00", "ieQTL2": "#0072B2", "sum": "#009E73"}
        ...
        self.outdir = os.path.join(
            str(os.path.dirname(os.path.abspath(__file__))), "picalo_graphical_overview"
        )
    ...
    def start(self):
        ...
        ieqtl1, betas1 = self.generate_ieqtl_model(
            maf=self.maf1,
            base_matrix=X,
        ...
        ieqtl2, betas2 = self.generate_ieqtl_model(
            maf=self.maf2,
            base_matrix=X,
        ...
        self.plot_log_likelihood(
            context=X.loc[self.example_name, "context"],
        ...
        ieqtl1_opt["y_hat"] = self.predict(
            X=ieqtl1_opt[["intercept", "genotype", "context", "interaction"]],
            betas=betas1,
        )
        ...
        ieqtl2_opt["y_hat"] = self.predict(
            X=ieqtl2_opt[["intercept", "genotype", "context", "interaction"]],
            betas=betas2,
        )
    ...
    def generate_base_model(self):
        ...
        X.index = [
            x if i != self.example_index else self.example_name
            for i, x in enumerate(X.index)
        ]
    ...
    def generate_ieqtl_model(self, maf, base_matrix, error_sd, betas, base_seed=0):
        ...
        genotype = np.array(
            [0] * round((1 - maf) ** 2.0 * self.n_points)
        ...
        X["expression"] = X.loc[
            :, ["intercept", "genotype", "context", "interaction"]
        ].dot(betas) + np.random.normal(0, error_sd, self.n_points)

        # Model the ieQTL.
        betas = self.fit(
            X=X[["intercept", "genotype", "context", "interaction"]], y=X["expression"]
        )
        X["y_hat"] = self.predict(
    ...
    def optimize_ieqtl(self, X, betas):
        ...
        y_values_df = pd.DataFrame(
            np.nan, index=eval_df.index, columns=[self.x1, self.x3]
        )
        for eval_pos in [self.x1, self.x3]:
            # Replace the context with the evaluation position and recalculate
            # the interaction term.
            ...
            # Calculate the y_hat of the model for the original betas.
            eval_df["adj_y_hat"] = self.predict(
                X=eval_df[["intercept", "genotype", "context", "interaction"]],
                betas=betas,
            )
        ...
        # Determine the coefficients.
        coef_a, coef_b = self.calc_parabola_vertex(
            x1=self.x1,
            x2=X["context"],
            x3=self.x3,
            y1=y_values_df[self.x1],
            ...
            if len(subset.index) > 1:
                if self.example_name not in subset.index or 1 == 1:
                    sns.regplot(
                        ...
                        data=subset,
                        scatter_kws={
                            "facecolors": self.color_map[group_id],
                            "s": 50,
                            "linewidth": 0,
                            "alpha": 0.8,
                        },
                        line_kws={"color": self.color_map[group_id], "alpha": 1},
                        ax=ax,
                    )
                else:
                    sns.regplot(
                        ...
                        y="expression",
                        data=subset.loc[
                            [x for x in subset.index if x != self.example_name], :
                        ],
                        scatter_kws={
                            "facecolors": "#000000",
                            "s": 50,
    ...
    def plot_log_likelihood(
        ...
        line_data = []
        point_data = []
        for a, b, xlim, label in [
            (a1, b1, xlim1, "ieQTL1"),
            (a2, b2, xlim2, "ieQTL2"),
            (
                a1 + a2,
                b1 + b2,
                (min(xlim1[0], xlim2[0]), max(xlim1[1], xlim2[1])),
                "sum",
            ),
        ]:
            ...
            point_data.append(
                [label, vertex_xpos, a * vertex_xpos**2 + b * vertex_xpos + 0]
            )
        ...
        for i, (a, b, label) in enumerate(
            [(a1, b1, "ieQTL1"), (a2, b2, "ieQTL2"), (a1 + a2, b1 + b2, "sum")]
        ):
            color = "#000000"
            if palette is not None:
                color = palette[label]
