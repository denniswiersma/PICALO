...
class main:
    ...
    def create_argument_parser(self):
        ...
        parser.add_argument(
            "-picalo",
            action="store_true",
            help="Observed result is from PICALO. " "Default: False.",
        )
        parser.add_argument(
            "-ex",
            "--extension",
        ...
        parser.add_argument(
            "-of", "--outfile", type=str, required=True, help="The name of the outfile."
        )
    ...
    def start(self):
        ...
        print("Loading data.")
        simulated_beta_df = self.load_file(
            inpath=os.path.join(self.simulation_path, "model_betas.txt.gz"),
            header=0,
            index_col=0,
            nrows=nrows,
        )
        simulated_std_df = self.load_file(
            inpath=os.path.join(self.simulation_path, "model_std.txt.gz"),
            header=0,
            index_col=0,
            nrows=nrows,
        )
        ...
        simulated_rsquared_df = self.load_file(
            inpath=os.path.join(self.simulation_path, "model_rsquared.txt.gz"),
            header=0,
            index_col=0,
            nrows=nrows,
        )
        ...
        simulated_beta_df = simulated_beta_df.merge(
            simulated_rsquared_df, left_index=True, right_index=True
        )
        ...
        if self.picalo:
            observed_df = self.load_file(
                inpath=os.path.join(
                    self.observation_path, "PIC_interactions", "PIC1.txt.gz"
                ),
                header=0,
                index_col=None,
                nrows=nrows,
            ...
            for prefix in ["beta", "std"]:
                observed_df.columns = [
                    col.replace(
                        "{}-covariate".format(prefix),
                        "{}-hidden_covariate0".format(prefix),
                    ).replace(
                        "{}-interaction".format(prefix),
                        "{}-hidden_covariate_interaction0".format(prefix),
                    )
            ...
            observed_df = self.load_file(
                inpath=os.path.join(self.observation_path, "eQTLSummaryStats.txt.gz"),
                header=0,
                index_col=None,
                nrows=nrows,
            )
            for prefix in ["beta", "std"]:
                observed_df.columns = [
                    col.replace(
                        "{}-hidden_covariate".format(prefix),
                        "{}-hidden_covariate_interaction".format(prefix),
                    ).replace("Xgenotype", "")
                    if (
                        col.startswith("{}-hidden_covariate".format(prefix))
                        and col.endswith("Xgenotype")
                    )
                    else col
                    for col in observed_df.columns
                ]
        ...
        observed_beta_df = observed_df.loc[
            :,
            [
                col
                for col in observed_df.columns
                if col.startswith("beta-") or col == "r-squared"
            ],
        ]
        observed_beta_df.columns = [
            col.replace("beta-", "") for col in observed_beta_df.columns
        ]

        observed_std_df = observed_df.loc[
            :, [col for col in observed_df.columns if col.startswith("std-")]
        ]
        if not self.picalo:
            observed_std_df["r-squared"] = 0
        observed_std_df.columns = [
            col.replace("std-", "") for col in observed_std_df.columns
        ]
        del observed_df
        ...
        overlapping_terms = [
            col for col in simulated_beta_df.columns if col in observed_beta_df.columns
        ]
        print("Terms:")
        ...
        self.plot_model_comparison(
            simulated_beta_df=simulated_beta_df,
    ...
    def plot_model_comparison(
        ...
        for i in range(ncols * nrows):
            ...
            if i < len(overlapping_terms):
                ...
                compare_df = (
                    simulated_beta_df[[term]]
                    .merge(simulated_std_df[[term]], left_index=True, right_index=True)
                    .merge(observed_beta_df[[term]], left_index=True, right_index=True)
                    .merge(observed_std_df[[term]], left_index=True, right_index=True)
                )
                compare_df.columns = [
                    "simulated-beta",
                    "simulated-std",
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir,
                    "{}_ModelComparison.{}".format(self.out_filename, extension),
                )
            )
    ...
    def plot_delta_per_stats(self, delta_beta_df, genotype_stats_df):
        ...
        for row_index, model_term in enumerate(model_terms):
            for col_index, stats_term in enumerate(stats_terms):
                ...
                compare_df = genotype_stats_df[[stats_term]].merge(
                    delta_beta_df[[model_term]], left_index=True, right_index=True
                )
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir,
                    "{}_DeltaVsGenotypeStats.{}".format(self.out_filename, extension),
                )
            )
        ...
        if n > 0:
            if n > 1:
                ...
                if include_concordance:
                    upper_left_quadrant = df.loc[
                        (df[scatter_x] < 0) & (df[scatter_y] > 0), :
                    ]
                    upper_right_quadrant = df.loc[
                        (df[scatter_x] > 0) & (df[scatter_y] > 0), :
                    ]
                    lower_left_quadrant = df.loc[
                        (df[scatter_x] < 0) & (df[scatter_y] < 0), :
                    ]
                    lower_right_quadrant = df.loc[
                        (df[scatter_x] > 0) & (df[scatter_y] < 0), :
                    ]

                    if coef < 0:
                        n_concordant = (
                            upper_left_quadrant.shape[0] + lower_right_quadrant.shape[0]
                        )
                        concordance = (100 / n) * n_concordant
                    elif coef > 0:
                        n_concordant = (
                            lower_left_quadrant.shape[0] + upper_right_quadrant.shape[0]
                        )
                        concordance = (100 / n) * n_concordant
                    else:
                        pass
