...
class main:
    def __init__(self):
        ...
        if self.palette_path is not None:
            ...
            f.close()
            self.palette = {
                "Comp{}".format(i): palette["PIC{}".format(i + 1)] for i in range(0, 25)
            }
    ...
    def start(self):
        ...
        for fpath in glob.glob(
            os.path.join(self.indir, "output", "{}_*Samples".format(self.folder))
        ):
            n_samples = int(
                os.path.basename(fpath)
                .replace(self.folder + "_", "")
                .replace("Samples", "")
            )
            geno_stats_inpath = os.path.join(fpath, "genotype_stats.txt.gz")
            cov_selection_inpath = os.path.join(
                fpath, "PIC1", "covariate_selection.txt.gz"
            )
            if not os.path.exists(geno_stats_inpath) or not os.path.exists(
                cov_selection_inpath
            ):
                continue
