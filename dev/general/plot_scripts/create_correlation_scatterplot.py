...
class main:
    ...
    def start(self):
        ...
        print("Correlating.")
        corr_df = self.correlate(index_df=row_df, columns_df=col_df, triangle=triangle)
        corr_df["abs coef"] = corr_df["coef"].abs()
        # corr_df["group"] = corr_df["variable1"].str.split(".", n=1, expand=True)[0]
        corr_df["group"] = [
            self.ct_trans_dict[ct]
            for ct in corr_df["variable1"].str.split("_", n=1, expand=True)[0]
        ]
        ...
        self.plot(
            ...
            group="group",
            order=[
                col for col in col_df.columns if col in corr_df["variable2"].unique()
            ],
            annot=None,
    ...
    def plot(
        ...
        sns.boxplot(x=x, y=y, hue=group, data=df, order=order, palette=palette, ax=ax)
