...
class main:
    ...
    def start(self):
        ...
        for dataset in std_df["dataset"].unique():
            ...
            if dataset != "":
                ...
                dataset_samples = std_df.loc[
                    std_df["dataset"] == dataset, "sample"
                ].unique()
            ...
            if triangle:
                ...
                print(
                    "Lower triangle correlations = mean: {:.4f}\tstd {:.4f}".format(
                        np.nanmean(corr_values), np.nanstd(corr_values)
                    )
                )
            else:
                corr_values = np.abs(corr_m)
                print(
                    "All correlations = mean: {:.4f}\tstd {:.4f}".format(
                        np.nanmean(corr_values), np.nanstd(corr_values)
                    )
                )
    ...
    def correlate(self, index_df, columns_df, triangle=False):
        ...
        index_df_colnames = [
            ...
            for colname in index_df.columns
        ]
        column_df_colnames = [
            "{} [N={:,}]".format(colname, column_df_n_nan_values[colname])
            for colname in columns_df.columns
        ]

        corr_df = pd.DataFrame(
            np.nan, index=index_df_colnames, columns=column_df_colnames
        )
        pvalue_df = pd.DataFrame(
            np.nan, index=index_df_colnames, columns=column_df_colnames
        )

        for i, (index_column, index_colname) in enumerate(
            zip(index_df.columns, index_df_colnames)
        ):
            for j, (column_column, column_colname) in enumerate(
                zip(columns_df.columns, column_df_colnames)
            ):
                if triangle and i < j:
                    continue
                corr_data = pd.concat(
                    [index_df[index_column], columns_df[column_column]], axis=1
                )
