...
class main:
    ...
    def start(self):
        ...
        for pic_index in range(1, max_pic + 1):
            ...
            print(f"len of eqtls_loaded {len(eqtls_loaded)}")
            for row_index, eqtl_id in enumerate(eqtls_loaded):
                x = len(pic_plot_ids)
                y = 0

                if row_index == 0:
                    print(f"eqtl_id: {eqtl_id}")
                    print(f"pic_plot_ids: {pic_plot_ids}")

                if eqtl_id not in pic_plot_ids:
                    # print(f"skipping {eqtl_id}")
                    # print(f"skipping {pic_plot_ids}")
                    y += 1
                    continue
            ...
            print(f"skipped {y}/{x} eqtls/ids/whatever")
