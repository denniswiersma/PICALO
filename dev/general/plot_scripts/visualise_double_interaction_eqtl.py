...
class main:
    def __init__(self):
        ...
        self.outdir = os.path.join(
            str(Path(__file__).parent.parent), "visualise_double_interaction_eqtl"
        )
        ...
        for row_index, (_, row) in enumerate(eqtl_df.iterrows()):
            ...
            for cov1, cov2 in eqtl_covs:
                ...
                for i, cov_df, cov in ((1, cov1_df, cov1), (2, cov2_df, cov2)):
                    ...
                    data["interaction{}".format(i)] = (
                        data["genotype"] * data["covariate{}".format(i)]
                    )
                ...
                interaction_pvalue1 = (
                    OLS(
                        data["expression"],
                        data[["intercept", "genotype", "covariate1", "interaction1"]],
                    )
                    .fit()
                    .pvalues[3]
                )
                ...
                context_genotype_r1, _ = stats.pearsonr(
                    data["genotype"], data["covariate1"]
                )

                interaction_pvalue2 = (
                    OLS(
                        data["expression"],
                        data[["intercept", "genotype", "covariate2", "interaction2"]],
                    )
                    .fit()
                    .pvalues[3]
                )
                ...
                context_genotype_r2, _ = stats.pearsonr(
                    data["genotype"], data["covariate2"]
                )
