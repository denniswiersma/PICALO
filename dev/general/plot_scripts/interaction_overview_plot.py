...
class main:
    ...
    def start(self):
        ...
        n_ieqtls_unique_per_pic = [
            (name, value)
            for name, value in single_interaction_df.sum(axis=0).iteritems()
        ]
        ...
        # Construct pie data.
        data = (
            [no_interaction_df.shape[0]]
            + n_ieqtls
            + [multiple_interactions_df.shape[0]]
        )
        labels = ["None"] + pics + ["Multiple"]
        colors = None
        if self.palette is not None:
            colors = ["#D3D3D3"] + [self.palette[pic] for pic in pics] + ["#808080"]
        explode = [0.0] + [0.1 for _ in pics] + [0.1]

        data_sum = np.sum(data)
        for i in range(len(data)):
            print(
                "{} (N = {}) = {:.2f}%".format(
                    labels[i], data[i], (100 / data_sum) * data[i]
                )
            )

        total_n_ieqtls = pic_df.shape[0] - no_interaction_df.shape[0]
        print(
            "Total interactions (N = {}) = {:.2f}%".format(
                total_n_ieqtls, (100 / pic_df.shape[0]) * total_n_ieqtls
            )
        )
        ...
        self.plot(
            data=data, labels=labels, explode=explode, colors=colors, extension="png"
        )
        self.plot(
            data=data,
            labels=labels,
        ...
        for n_inter in range(
            min(n_interactions.unique()), max(n_interactions.unique()) + 1
        ):
            if n_inter > 0:
    ...
    def plot(
        ...
        fig.savefig(
            os.path.join(
                self.outdir,
                "{}_interaction_piechart.{}".format(self.out_filename, extension),
            )
        )
        plt.close()
