...
class main:
    ...
    def plot(
        self, x_df, y_df, sa_df=None, palette=None, color="#000000", ci=95, lines=True
    ):
        ncols = x_df.shape[1]
        nrows = y_df.shape[1]
        ...
        for i, y_col in enumerate(y_df.columns):
            for j, x_col in enumerate(x_df.columns):
                ...
                df = x_df[[x_col]].merge(
                    y_df[[y_col]], left_index=True, right_index=True
                )
                df.columns = ["x", "y"]
                facecolors = "#808080"
                if sa_df is not None and palette is not None:
                ...
                ax.set_title("", fontsize=22, color=color, weight="bold")
        ...
        for extension in self.extensions:
            fig.savefig(
                os.path.join(
                    self.outdir,
                    "{}_comparison_scatterplot2.{}".format(
