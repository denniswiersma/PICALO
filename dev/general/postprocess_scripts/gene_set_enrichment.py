...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="The covariates to analyse." "Default: all.",
        )
        parser.add_argument(
            "-gc",
            "--gene_correlations",
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="The minimal average expression of a gene." "Default: 0.",
        )
        parser.add_argument(
            "-mc",
            "--min_corr",
            type=float,
            default=0,
            help="The minimal correlation of a gene " "for inclusion. Default 0.",
        )
        parser.add_argument(
            "-a",
            "--alpha",
    ...
    def start(self):
        ...
        if self.min_avg_expression > 0:
            print(
                "\tFiltering genes with >{} average gene expression".format(
                    self.min_avg_expression
                )
            )
        ...
        if self.protein_coding_path is not None:
            ...
            protein_coding_df = self.load_file(
                self.protein_coding_path, header=None, index_col=None
            )
            pre_shape = df.shape[0]
            protein_coding_df.index = protein_coding_df.iloc[:, 0].str.split(
                ".", n=1, expand=True
            )[0]
        ...
        for ieqtl_result_path in ieqtl_result_paths:
            ...
            ieqtl_result_df = self.load_file(
                ieqtl_result_path, header=0, index_col=None
            )
            if ieqtl_result_df.shape[0] != len(ieqtl_result_df["gene"].unique()):
                print("Non unique genes in PIC interaction output.")

            ieqtl_result_df.index = [
                index.split(".")[0] for index in ieqtl_result_df["gene"]
            ]
            ...
            if (
                len(
                    {snp_col, beta_geno_col, beta_inter_col, fdr_col}.intersection(
                        set(ieqtl_result_df.columns)
                    )
                )
                != 4
            ):
            ...
            ieqtl_result_df["direction"] = (
                (ieqtl_result_df[beta_geno_col] * ieqtl_result_df[beta_inter_col]) > 0
            ).map({True: "induces", False: "inhibits"})
            if len(ieqtl_results) == 0:
                ieqtl_result_df = ieqtl_result_df[[snp_col, fdr_col, "direction"]]
                ieqtl_result_df.columns = [
                    "SNP",
        ...
        for pvalue_col in [col for col in df.columns if "pvalue" in col]:
            df[pvalue_col.replace("pvalue", "FDR")] = multitest.multipletests(
                df[pvalue_col], method="fdr_bh"
            )[1]

        print("Saving file.")
        self.save_file(df=df, outpath=os.path.join(self.file_outdir, "info.txt.gz"))
        # exit()
        #
        # print("Loading files.")
        ...
        covariates = [
            col.replace(" FDR", "") for col in df.columns if col.endswith(" FDR")
        ]
        ...
        for covariate in covariates:
            ...
            for correlation_direction in ["positive", "negative"]:
                ...
                subset_df["abs correlation zscore"] = subset_df[
                    "correlation zscore"
                ].abs()
                subset_df.sort_values(
                    by="abs correlation zscore", ascending=False, inplace=True
                )

                if correlation_direction == "positive":
                    subset_df = subset_df.loc[
                        (subset_df["correlation zscore"] > 0)
                        & (subset_df["abs correlation zscore"] > self.min_corr),
                        :,
                    ]
                ...
                corr_subset_df = corr_subset_df.loc[
                    corr_subset_df["correlation FDR"] < self.alpha, :
                ]
                if corr_subset_df.shape[0] > 0:
                    print(
                        "\t{} correlation (abs r > {:.2f})".format(
                            correlation_direction, self.min_corr
                        )
                    )
                    ...
                    self.save_file(
                        ...
                        outpath=os.path.join(
                            covariate_outdir,
                            "toppgene_enrichment_{}_{}_data.txt.gz".format(
                                covariate, correlation_direction
                            ),
                        ),
                    )
                    ...
                    self.save_file(
                        ...
                        outpath=os.path.join(
                            covariate_outdir,
                            "toppgene_enrichment_{}_{}.txt.gz".format(
                                covariate, correlation_direction
                            ),
                        ),
                    )
                    ...
                    if enrich_df.shape[0] > 0:
                        ...
                        enrich_df["avg_abs_correlation_zscore_inTerm"] = [
                            corr_subset_df.loc[
                                corr_subset_df["symbol"].isin(genes.split(", ")),
                                "correlation zscore",
                            ]
                            .abs()
                            .mean()
                            for genes in enrich_df["Genes"]
                        ]
                        enrich_df["avg_abs_correlation_zscore_Overall"] = (
                            corr_subset_df["correlation zscore"].abs().mean()
                        )
                ...
                signif_subset_df = (
                    subset_df.loc[subset_df["ieQTL FDR"] < self.alpha, :]
                    .iloc[: self.top_n, :]
                    .copy()
                )
                if signif_subset_df.shape[0] > 0:
                    print(
                        "\t{} correlation (ieQTL FDR < {})".format(
                            correlation_direction, self.alpha
                        )
                    )
                    ...
                    if enrich_df.shape[0] > 0:
                        ...
                        enrich_df["correlation_direction"] = correlation_direction
                        enrich_df["avg_abs_correlation_zscore_inTerm"] = [
                            signif_subset_df.loc[
                                subset_df["symbol"].isin(genes.split(", ")),
                                "correlation zscore",
                            ]
                            .abs()
                            .mean()
                        ...
                        enrich_df["avg_abs_correlation_zscore_Overall"] = (
                            signif_subset_df["correlation zscore"].abs().mean()
                        )
    ...
    def toppgene_gene_lookup(genes, chunk_size=25000):
        chuncks = [
            genes[i * chunk_size : (i + 1) * chunk_size]
            for i in range((len(genes) + chunk_size - 1) // chunk_size)
        ]
    ...
    def toppgene_func_enrichment(self, entrez_ids, n_print=4):
        ...
        data = json.dumps(
            {
                "Genes": [int(entrez_id) for entrez_id in entrez_ids],
                "Categories": [
                    {"Type": "Pathway", "PValue": self.alpha, "Correction": "FDR"},
                    {"Type": "ToppCell", "PValue": self.alpha, "Correction": "FDR"},
                ],
            }
        )
        response = requests.post(
        ...
        if df.shape[0] > 0:
            df["Genes"] = [
                ", ".join([gene["Symbol"] for gene in value]) for value in df["Genes"]
            ]
