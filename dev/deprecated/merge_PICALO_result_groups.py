...
class main:
    ...
    def start(self):
        ...
        for pic, columns in self.groups.items():
            fpath = os.path.join(
                self.indir, self.name + "-PIC" + pic, "data", "PICBasedOnPCX.txt.gz"
            )
            ...
            for i, colname in enumerate(subset.columns):
                ...
                if self.selected[pic] == colname:
                    appendix = "-X"
                new_colnames.append(
                    "PIC{}-{}-Group{}{}".format(pic, colname, i, appendix)
                )
        ...
        command = [
            ...
            "PIC_groups",
            "-o",
            "2021-11-24-BIOS-BIOS-cis-NoRNAPhenoNA-NoSexNA-NoMixups-NoMDSOutlier-NoRNAseqAlignmentMetrics-PICGroups",
        ]
