...
class main:
    def __init__(self):
        ...
        self.data_outdir = os.path.join(
            base_dir, "merge_PICALO_job_results", output, "data"
        )
        self.plot_outdir = os.path.join(
            base_dir, "merge_PICALO_job_results", output, "plot"
        )
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="The name of the jobs. Replace the PC index" "with <N>",
        )
        parser.add_argument(
            "-p", "--pic", type=int, required=False, default=None, help="The PIC."
        )
        parser.add_argument(
            "-o",
            "--output",
    ...
    def start(self):
        ...
        for pc_index in np.arange(1, 26):
            fpath = os.path.join(
                self.indir,
                self.job_name + "-PC{}AsCov".format(pc_index),
                "PIC1",
                "info.txt.gz",
            )
            if os.path.exists(fpath):
                ...
                info_df_m = info_df.melt(
                    id_vars=["index", "covariate", "component", "group"]
                )
        ...
        command = [
            "python3",
            "/groups/umcg-biogen/tmp01/output/2020-11-10-PICALO/dev/plot_scripts/create_correlation_heatmap.py",
            "-rd",
            pic_df_path,
            "-rn",
            self.job_name,
        ...
        command = [
            ...
            "cell fractions %",
            "-o",
            self.job_name + "_vs_CellFractionPercentages",
        ]
        self.run_command(command)
        ...
        command = [
            ...
            "all STAR metrics",
            "-o",
            self.job_name + "_vs_AllSTARMetrics",
        ]
        self.run_command(command)
        ...
        command = [
            ...
            self.job_name,
            "-cd",
            "/groups/umcg-biogen/tmp01/output/2020-11-10-PICALO/preprocess_scripts/correlate_samples_with_avg_gene_expression/BIOS_CorrelationsWithAverageExpression.txt.gz",
            "-cn",
            "AvgExprCorrelation",
            "-o",
            self.job_name + "_vs_AvgExprCorrelation",
        ]
        self.run_command(command)
        ...
        command = [
            ...
            self.job_name,
            "-cd",
            "/groups/umcg-bios/tmp01/projects/PICALO/preprocess_scripts/prepare_bios_picalo_files/BIOS-BIOS-cis-NoRNAPhenoNA-NoSexNA-NoMixups-NoMDSOutlier-NoRNAseqAlignmentMetrics/SP140.txt.gz",
            "-cn",
            "SP140",
            "-o",
            self.job_name + "_vs_SP140",
        ]
        self.run_command(command)
