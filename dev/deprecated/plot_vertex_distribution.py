...
class main:
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            help="The color column(s) name in the -sa / " "--sample_annotation file.",
        )
        ...
        parser.add_argument(
            ...
            required=required,
            default=None,
            help="The sample column name in the -sa / " "--sample_annotation file.",
        )
    ...
    def start(self):
        ...
        if self.sa_path is not None:
            sa_df = self.load_file(
                self.sa_path, header=0, index_col=0, low_memory=False
            )
            for group in sa_df[self.color_id].unique():
                sample_group_dict[group] = sa_df.loc[
                    sa_df[self.color_id] == group, self.sample_id
                ].values.tolist()
        else:
        ...
        for group, samples in sample_group_dict.items():
            ...
            overlap_samples = [
                x
                for x in samples
                if x in vertex_df.columns and x in optimized_df.columns
            ]
            ...
            self.plot(
                box_df=box_subset_m,
                point_df=point_subset_m,
                color=self.palette[group],
                title=group,
                xlabel="samples",
                ylabel="vertex x-pos",
                appendix="_{}".format(group),
            )
            self.plot(
                box_df=box_subset_m,
                point_df=point_subset_m,
                ...
                xlabel="samples",
                ylabel="vertex x-pos",
                appendix="_{}_Zoomed".format(group),
            )
    ...
    def plot(
        ...
        sns.scatterplot(x=x, y=y, color="#000000", data=box_df, ax=ax)
        ...
        sns.scatterplot(x=x, y=y, color=color, data=point_df, s=25, ax=ax)
        ...
        fig.savefig(
            os.path.join(self.outdir, "vertex_x_distribution{}.png".format(appendix))
        )
        plt.close()
