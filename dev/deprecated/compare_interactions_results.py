...
class main:
    ...
    def start(self):
        ...
        gene_info_df = pd.read_csv(
            "/groups/umcg-bios/tmp01/projects/PICALO/data/ArrayAddressToSymbol.txt.gz",
            sep="\t",
            header=0,
            index_col=None,
        )
        ...
        print(
            "\t{} = {} PICs: n-ieQTLs: {:,}    {:.2f}% of eQTLs has an interaction".format(
                self.name1,
                df1.shape[1],
                df1_n_ieqtls,
                (100 / df1.shape[0]) * df1_interaction_df.shape[0],
            )
        )
        print(
            "\t{} = {} PCs:  n-ieQTLs: {:,}    {:.2f}% of eQTLs has an interaction".format(
                self.name2,
                df2.shape[1],
                df2_n_ieqtls,
                (100 / df2.shape[0]) * df2_interaction_df.shape[0],
            )
        )

        n_ieqtls_df1_to_df2 = len(
            [x for x in df1_interaction_df.index if x in df2_interaction_df.index]
        )
        n_ieqtls_df2_to_df1 = len(
            [x for x in df2_interaction_df.index if x in df1_interaction_df.index]
        )
        overlap_eqtls_with_interaction = set(df1_interaction_df.index).intersection(
            set(df2_interaction_df.index)
        )
        print(
            "\t{:,}/{:,} of the {} ieQTLs also interacts with one or more {}".format(
                n_ieqtls_df1_to_df2, df1_interaction_df.shape[0], self.name1, self.name2
            )
        )
        print(
            "\t{:,}/{:,} of the {} ieQTLs also interacts with one or more {}".format(
                n_ieqtls_df2_to_df1, df2_interaction_df.shape[0], self.name2, self.name1
            )
        )
        print(
            "\tOverlap in eQTLs with an interaction: {:,}".format(
                len(overlap_eqtls_with_interaction)
            )
        )
    ...
    def create_replication_df(df1, df2):
        ...
        replication_df = pd.DataFrame(
            np.nan, index=df1_subset.columns, columns=df2_subset.columns
        )
        for cov1 in df1_subset.columns:
            cov1_signif_df = df2_subset.loc[df1_subset.loc[:, cov1] <= 0.05, :]
            for cov2 in df2_subset.columns:
                replication_df.loc[cov1, cov2] = (
                    cov1_signif_df.loc[:, cov2] <= 0.05
                ).sum() / cov1_signif_df.shape[0]

        df1_n_signif = (df1_subset <= 0.05).sum(axis=0)
        df2_n_signif = (df2_subset <= 0.05).sum(axis=0)

        replication_df.index = [
            "{} [n={}]".format(ct, df1_n_signif.loc[ct]) for ct in df1_subset.columns
        ]
        replication_df.columns = [
            "{} [n={}]".format(ct, df2_n_signif.loc[ct]) for ct in df2_subset.columns
        ]
    ...
    def plot_heatmap(self, df, xlabel="", ylabel="", filename=""):
        ...
        fig.savefig(
            os.path.join(
                self.outdir, "{}_{}_heatmap.png".format(self.out_filename, filename)
            )
        )
        plt.close()
