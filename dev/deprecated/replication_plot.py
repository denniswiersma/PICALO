...
class main:
    ...
    def start(self):
        print("Loading data")
        d_tvalue_df, d_pvalue_df = self.load_pic_interaction_data(
            indir=self.d_fim_indir
        )
        r_tvalue_df, r_pvalue_df = self.load_pic_interaction_data(
            indir=self.r_fim_indir
        )
        print(d_tvalue_df)
        print(r_tvalue_df)
        ...
        d_fdr_df = pd.DataFrame(
            np.nan, index=d_pvalue_df.index, columns=d_pvalue_df.columns
        )
        for colname in d_pvalue_df.columns:
            d_fdr_df.loc[:, colname] = multitest.multipletests(
                d_pvalue_df.loc[:, colname], method="fdr_bh"
            )[1]

        r_fdr_df = pd.DataFrame(
            np.nan, index=r_pvalue_df.index, columns=r_pvalue_df.columns
        )
        for colname in r_pvalue_df.columns:
            mask = d_fdr_df.loc[:, colname] <= 0.05
            r_fdr_df.loc[mask, colname] = multitest.multipletests(
                r_pvalue_df.loc[mask, colname], method="fdr_bh"
            )[1]
        del d_pvalue_df, r_pvalue_df

        print("Merging data.")
        d_df_m = self.melt_and_merge(
            tvalue_df=d_tvalue_df, fdr_df=d_fdr_df, name="discovery"
        )
        r_df_m = self.melt_and_merge(
            tvalue_df=r_tvalue_df, fdr_df=r_fdr_df, name="replication"
        )
        ...
        df_m = df_m.loc[
            (df_m["discovery FDR"] <= 0.05) & (df_m["replication FDR"] <= 0.05), :
        ]
        print(df_m)
        pic_replicating_interaction_counts = list(
            zip(*np.unique(df_m["variable"], return_counts=True))
        )
        pic_replicating_interaction_counts.sort(key=lambda x: -x[1])
        pics_with_replication_interactions = [
            x[0] for x in pic_replicating_interaction_counts if x[1] > 2
        ]
        columns = [
            "PIC{}".format(i)
            for i in range(1, 50)
            if "PIC{}".format(i) in pics_with_replication_interactions
        ]

        print("Plotting data")
        self.replication_regplot(
            df=df_m,
            col=columns,
            x="discovery t-value",
    ...
    def load_pic_interaction_data(self, indir):
        ...
        for i in range(1, 50):
            ...
            if os.path.exists(fpath):
                ...
                df["ieQTL tvalue-interaction"] = (
                    df["ieQTL beta-interaction"] / df["ieQTL std-interaction"]
                )
    ...
    def melt_and_merge(tvalue_df, fdr_df, name):
        ...
        tvalue_df_m = tvalue_df.melt(
            id_vars=["index"], value_name="{} t-value".format(name)
        )
    ...
    def replication_regplot(
        self,
        df,
        col,
        col_colname="variable",
        x="x",
        y="y",
        xlabel="",
        ylabel="",
        ...
        for i in range(ncols * nrows):
            ...
            if i < nplots:
                ...
                concordance = (100 / plot_df.shape[0]) * (
                ...
                ax.set_title(col[i], fontsize=25, fontweight="bold")
