...
class main:
    def __init__(self):
        ...
        self.technical_covariates_outdir = os.path.join(
            self.outdir, "technical_covariates"
        )
        ...
        for dir in [
            self.outdir,
            self.technical_covariates_outdir,
            self.jobs_dir,
    ...
    def create_argument_parser():
        ...
        parser.add_argument(
            ...
            required=False,
            default=1,
            help="The number of components to extract. " "Default: 1.",
        )
        parser.add_argument(
            "-min_iter",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=5,
            help="The expression PC step size to use." "Default: 5.",
        )
        parser.add_argument(
            "-st",
            "--sleep_time",
    ...
    def start(self):
        ...
        for end_pos in np.arange(
            self.step_size, tcov_df.shape[0] + self.step_size, self.step_size
        ):
            label = "{}ExpressionPCs".format(end_pos)
            outpath = os.path.join(
                self.technical_covariates_outdir, "{}.txt.gz".format(label)
            )
        ...
        for label, tech_covariate_path in pcs.items():
            ...
            jobfile_path = self.create_job_file(
                job_name=job_name,
        ...
        while True:
            ...
            if (
                ...
                or n_completed == n_jobs
            ):
                print(
                    "\t{:,}/{:,} jobs finished [{:.2f}%]".format(
                        n_completed, n_jobs, (100 / n_jobs) * n_completed
                    )
                )
        ...
        self.save_file(
            df=cov_select_df,
            outpath=os.path.join(self.outdir, "CovariateSelection.txt.gz"),
        )
        self.save_file(
            df=summary_stats_df,
        ...
        self.save_file(
            df=pics_df, outpath=os.path.join(self.outdir, "PICBasedOnPCX.txt.gz")
        )
        print("")
        ...
        print("  Plotting.")
        self.plot_covariate_selection(
            cov_select_df=cov_select_df, palette=covariate_palette, name=self.outfolder
        )
        for pic_groups, palette, appendix in zip(
            [pic_cov_groups, pic_corr_groups],
    ...
    def create_job_file(self, job_name, tech_covariate_path, jobs_dir, jobs_output_dir):
        ...
        lines.extend(
            [
                "  -tci {} \\".format(self.tech_covariate_with_inter),
    ...
    def plot_covariate_selection(self, cov_select_df, palette, name):
        ...
        cov_select_dfm.loc[
            ~cov_select_dfm["covariate"].isin(palette.keys()), "covariate"
        ] = ""
        cov_select_dfm["variable"] = (
            cov_select_dfm["variable"]
            .str.split("ExpressionPCs", n=1, expand=True)[0]
            .astype(int)
        )
        self.lineplot(
            df_m=cov_select_dfm,
    ...
    def plot_summary_stats(
        self, summary_stats_df, pic_groups, palette, name, appendix=""
    ):
        summary_stats_df["group"] = -1
        for group_index, group_indices in pic_groups.items():
            summary_stats_df.loc[
                summary_stats_df.index.isin(group_indices), "group"
            ] = group_index
